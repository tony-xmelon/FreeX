...
/// <summary>Coverage for the neutral dialog filter / format-choice composition.</summary>
public sealed class FileDialogFilterTests
{
    [Fact]
    public void FileFormatChoice_NormalizesExtensionWithLeadingDot()
    {
        var choice = new FileFormatChoice("Word documents", "docx");
        choice.Extension.Should().Be(".docx");
        choice.Pattern.Should().Be("*.docx");
    }

    [Fact]
    public void FileFormatChoice_PreservesExistingLeadingDot()
    {
        new FileFormatChoice("Word documents", ".docx").Extension.Should().Be(".docx");
    }

    [Fact]
    public void FileFormatChoice_RejectsBlankLabelAndExtension()
    {
        var act1 = () => new FileFormatChoice("  ", ".docx");
        var act2 = () => new FileFormatChoice("Word documents", "  ");
        act1.Should().Throw<ArgumentException>();
        act2.Should().Throw<ArgumentException>();
    }

    [Fact]
    public void Build_SingleChoice_ProducesWindowsStyleFilterWithAllFiles()
    {
        var filter = FileDialogFilter.Build([new FileFormatChoice("Word documents", ".docx")]);
        filter.Should().Be("Word documents (*.docx)|*.docx|All files (*.*)|*.*");
    }

    [Fact]
    public void Build_MultipleChoices_JoinsSegments()
    {
        var filter = FileDialogFilter.Build([
            new FileFormatChoice("Word documents", ".docx"),
            new FileFormatChoice("Rich text", ".rtf")
        ]);

        filter.Should().Be("Word documents (*.docx)|*.docx|Rich text (*.rtf)|*.rtf|All files (*.*)|*.*");
    }

    [Fact]
    public void Build_CanOmitAllFilesSegment()
    {
        var filter = FileDialogFilter.Build([new FileFormatChoice("Word documents", ".docx")], includeAllFiles: false);
        filter.Should().Be("Word documents (*.docx)|*.docx");
    }

    [Fact]
    public void Build_EmptyChoices_ReturnsAllFilesOnlyOrEmpty()
    {
        FileDialogFilter.Build([]).Should().Be("All files (*.*)|*.*");
        FileDialogFilter.Build([], includeAllFiles: false).Should().Be("");
    }

    [Fact]
    public void DefaultExtension_UsesFirstChoiceOrEmpty()
    {
        FileDialogFilter.DefaultExtension([new FileFormatChoice("Word documents", ".docx")]).Should().Be(".docx");
        FileDialogFilter.DefaultExtension([]).Should().Be("");
    }
}
