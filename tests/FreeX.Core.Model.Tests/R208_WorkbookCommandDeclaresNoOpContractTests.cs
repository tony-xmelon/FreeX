...
public sealed class R208_WorkbookCommandDeclaresNoOpContractTests
{
    ...
    [
        ...
        // named above and nothing else, so a post-hoc decision over both is complete rather than
        // partial: WorksheetAutoFilterColumnSync.Unchanged (content comparison of the stored
        // filterColumn entry, via WorksheetAutoFilterColumnComparison) plus FilterUndoSnapshot.Matches.
        ...
        // totals refresh is consulted through its OWN IsNoOp, which r245 made trustworthy; r231's
        // warning that "delegation propagates both" was what said to check that first.
        // ONE command remains: MoveRangeCommand, blocked on size (thirty snapshot fields) rather than
        // on any missing technique.
        ...
        // r258: SaveScenario and SaveCustomView are OFF this list. SaveTargetComparison compares both
        // records by CONTENT (strip the one list member, let record equality cover the scalars,
        // compare the list element-wise), which is the comparison r231 said was needed and did not
        // build. The probe confirmed r231's diagnosis exactly: with ==, both no-op tests fail and all
        // four changed-direction tests still pass -- a guard that looks like it works.
        // Its coverage contract also found a reference comparison reading would have missed:
        // ScenarioCellValue holds an abstract ScalarValue, whose RangeValue subtype carries an array.
