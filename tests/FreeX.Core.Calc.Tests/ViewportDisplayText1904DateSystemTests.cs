...
{
    [Fact]
    public void GetViewport_BuiltInShortDate_UsesCurrentWindowsShortDatePattern()
    {
        var culture = (CultureInfo)CultureInfo.GetCultureInfo("en-US").Clone();
        culture.DateTimeFormat.ShortDatePattern = "M/d/yyyy";
        using var cultureScope = TestCultureScope.CurrentCulture(culture);

        var workbook = new Workbook("test");
        var sheet = workbook.AddSheet("Sheet1");
        sheet.ColumnWidths[1] = 20;
        var styleId = workbook.RegisterStyle(new CellStyle { NumberFormat = "m/d/yy" });
        var cell = Cell.FromValue(new NumberValue(DateTimeValue.FromDateTime(new DateTime(2026, 8, 31)).Value));
        ...
        viewport.Cells.Single(cell => cell.Row == 1 && cell.Col == 1).DisplayText
            .Should().Be("8/31/2026");
    }
