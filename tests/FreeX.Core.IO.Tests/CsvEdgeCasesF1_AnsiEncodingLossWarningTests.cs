...
public sealed class CsvEdgeCasesF1_AnsiEncodingLossWarningTests
{
    ...
    public void CsvSaveWithWarnings_ReportsWarning_WhenTextCannotBeRepresentedInAnsiCodePage()
    {
        ...
        // The file itself is still written (warnings report data loss, they don't block the save),
        // and the on-disk bytes are unchanged from before the fix -- still the literal '?' fallback,
        // since the plain CSV/TXT format genuinely has no way to encode these characters. The emoji
        // is a surrogate pair, which .NET's replacement fallback collapses to a single '?' (not one
