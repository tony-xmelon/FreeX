...
// Enumerable.ElementAt -> ArgumentOutOfRangeException). Style-independent row attributes (height,
// hidden, outline, etc.) must still be preserved.
public sealed class XlsxWorksheetMetadataPreserverRowStyleTests
{
    private static readonly XNamespace Ns = "http://schemas.openxmlformats.org/spreadsheetml/2006/main";

    [Fact]
    public void MergeWorksheetRowAttributes_DoesNotCopyStaleRowStyleIndex()
    {
        ...
        // Style-independent layout attributes must still round-trip.
        targetRow.Attribute("ht")?.Value.Should().Be("27");
        targetRow.Attribute("customHeight")?.Value.Should().Be("1");
        targetRow.Attribute("hidden")?.Value.Should().Be("1");
    }
}
