...
public sealed class HeaderFooterEditorPlannerTests
{
    ...
    public void R127_ComposeTargetLabel_JoinsThroughTheResourceFormatDelegateInsteadOfHardcodingOrder()
    {
        ...
        {
            calls.Add((key, args));
            // Simulate a locale that reorders section-before-scope, proving the join order is
            // controlled by the resource, not hardcoded scope-then-section concatenation.
    ...
    public void R127_ComposeTargetLabel_FallsBackToSectionOnlyWhenScopeIsBlank()
    {
        ...
        formatterInvoked.Should().BeFalse("an empty scope label has no legitimate join order to preserve");
    }
