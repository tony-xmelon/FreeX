...
public sealed class R18_pagination_Tests
{
    ...
    /// body (~1026px for A4 narrow), not after a fixed count derived from the AVERAGE row height (28px
    /// average -&gt; ~36 rows/page, which would put ~1520px of real content on a ~1026px page).
    /// </summary>
    [Fact]
    public void BuildPlan_NonUniformRowHeights_BreaksFirstPageByAccumulatedHeightNotAverageCount()
    {
        ...
        // Real cumulative height: 10 rows @100px = 1000px, + row 11 @20px = 1020px (<= printable
        // ~1026.24px); row 12 would push it to 1040px, over budget -> the first page stops at row 11.
        plan.RowPlans[0].BodyRows[^1].Should().Be(11u,
            "10 rows@100px + 1 row@20px = 1020px fits the ~1026px printable body, but a 12th row would " +
            "overflow it -- the average-based (28px average -> ~36 rows/page) approach would instead put " +
            "36 rows (~1520px of real content) on this page");
    ...
    /// (~697.92px for A4 narrow), not after a fixed count derived from the AVERAGE column width.
    /// </summary>
    [Fact]
    public void BuildPlan_NonUniformColumnWidths_BreaksFirstPageByAccumulatedWidthNotAverageCount()
    {
        ...
        // printable ~697.92px); col4 would push it to 698px, just over budget -> the first page stops
        // at column 3. The average-based approach (average ~72.8px/col across all 50 columns -> ~9
        // columns/page) would instead put 9 columns on this page.
        plan.ColumnPlans[0].BodyColumns[^1].Should().Be(3u,
            "285+285+64 = 634px fits the ~697.92px printable body, but a 4th (64px) column would " +
            "overflow it");
