...
public sealed class Wave194AutoFilterMixedTypePhysicalSourceTests
{
    ...
    public void PhysicalSelector_PinsProductionChecklistClicksTransitionsAndPackagePostconditions()
    {
        ...
        Wave194AutoFilterMixedTypeGeometryAssertions.AssertBoundGeometry(probe);
        probe.Should().Contain("click_autofilter_control 74 319");
        probe.Should().Contain("click_autofilter_control \"$mixed_type_target_click_x_offset\" \"$mixed_type_target_click_y_offset\"");
        ...
        probe.Should().Contain("mixed_type_target_region_changed");
        probe.Should().Contain("$output/${prefix}-menu-open.png\" \"$output/${prefix}-menu-cleared.png\" 20");
