...
public sealed class AvaloniaInteractionCoverageTests
{
    ...
    public void RibbonSurfaceInventory_PreservesEveryDeclaredPlacement()
    {
        ...
        // 602 canonical shared placements plus the 42 runtime shape-gallery leaves. The canonical
        ...
        Assert.Equal(644, rows.Length);
        Assert.Equal(312, rows.Count(row => row.Kind != nameof(RibbonMenuItem)));
        Assert.Equal(332, rows.Count(row => row.Kind == nameof(RibbonMenuItem)));
        Assert.Equal(607, rows.Select(row => row.CommandId).Distinct().Count());
        ...
        Assert.Contains("$authoritativeRibbonBindingRowCount = 644", runner, StringComparison.Ordinal);
    ...
    public async Task RibbonBindingsSection_EmitsAllAuthoritativeRows()
    {
        await Session.Dispatch(async () =>
        {
            ...
            try
            {
                ...
                Assert.Equal(644, results.Count(result => result.Category == "ribbon-command"));
                Assert.Equal(74, results.Count(result => result.Category == "ribbon-collapsed-group"));
                Assert.Equal(718, results.Count);
