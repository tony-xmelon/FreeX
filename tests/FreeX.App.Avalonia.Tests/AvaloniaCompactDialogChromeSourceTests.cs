...
public sealed class AvaloniaCompactDialogChromeSourceTests
{
    ...
    public void SharedCompactChrome_CarriesTheExistingDialogMetrics()
    {
        ...
        source.Should().Contain("Color.Parse(CompactDialogVisualTokens.BorderHex)");
        source.Should().Contain("Color.Parse(CompactDialogVisualTokens.FieldBorderHex)");
        source.Should().Contain("Color.Parse(CompactDialogVisualTokens.DisabledForegroundHex)");
        source.Should().Contain("Color.Parse(CompactDialogVisualTokens.DisabledBorderHex)");
        source.Should().NotContain("public double ControlHeight { get; init; } = 24;");
        source.Should().NotContain("public double ButtonHeight { get; init; } = 26;");
        source.Should().NotContain("public double FontSize { get; init; } = 12;");
        source.Should().NotContain("public Thickness ButtonPadding { get; init; } = new(12, 3);");
        source.Should().NotContain("Color.FromRgb(200, 200, 200)");
        source.Should().NotContain("Color.FromRgb(183, 188, 194)");
