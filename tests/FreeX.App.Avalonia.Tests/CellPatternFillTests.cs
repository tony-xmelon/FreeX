...
/// <summary>
/// Unit tests for <see cref="CellSuperSubScript"/> — the font-adjustment math that mirrors
/// WPF's <c>ResolveSuperSubFontAdjustment</c> and constants in <c>GridView.cs</c>.
/// </summary>
public sealed class CellSuperSubScriptTests
{
    // ── IsActive ──────────────────────────────────────────────────────────────────────────────────

    [Fact]
    public void IsActive_NullStyle_ReturnsFalse() =>
        CellSuperSubScript.IsActive(null).Should().BeFalse();

    [Fact]
    public void IsActive_NeitherSet_ReturnsFalse()
    {
        var style = new CellStyle();
        CellSuperSubScript.IsActive(style).Should().BeFalse();
    }

    [Fact]
    public void IsActive_SuperscriptTrue_ReturnsTrue()
    {
        var style = new CellStyle { Superscript = true };
        CellSuperSubScript.IsActive(style).Should().BeTrue();
    }

    [Fact]
    public void IsActive_SubscriptTrue_ReturnsTrue()
    {
    ...
    [Fact]
    public void Constants_MatchWpfGridViewValues()
    {
        // WPF GridView.cs lines 487-489
        CellSuperSubScript.FontSizeFactor.Should().BeApproximately(0.583, precision: 0.001);
        CellSuperSubScript.SuperBaselineRatio.Should().BeApproximately(0.33, precision: 0.001);
        CellSuperSubScript.SubBaselineRatio.Should().BeApproximately(0.14, precision: 0.001);
    }

    // ── Resolve: no super/sub → identity ─────────────────────────────────────────────────────────

    [Fact]
    public void Resolve_NullStyle_ReturnsIdentity()
    {
        CellSuperSubScript.Resolve(null, 12.0, out var fontSize, out var offset);
        fontSize.Should().BeApproximately(12.0, precision: 0.001);
        offset.Should().Be(0);
    }

    [Fact]
    public void Resolve_NeitherSet_ReturnsIdentity()
    {
        var style = new CellStyle();
        CellSuperSubScript.Resolve(style, 14.0, out var fontSize, out var offset);
        fontSize.Should().BeApproximately(14.0, precision: 0.001);
        offset.Should().Be(0);
    }
    ...
    [Fact]
    public void Resolve_Superscript_ReducesFontSize()
    {
        var style = new CellStyle { Superscript = true };
        CellSuperSubScript.Resolve(style, 12.0, out var fontSize, out _);
        fontSize.Should().BeApproximately(12.0 * 0.583, precision: 0.001);
    }

    [Fact]
    public void Resolve_Superscript_NegativeOffset_ShiftsUp()
    {
        var style = new CellStyle { Superscript = true };
        CellSuperSubScript.Resolve(style, 12.0, out _, out var offset);
        // Negative = upward shift; magnitude = fontSize * SuperBaselineRatio
        offset.Should().BeApproximately(-(12.0 * 0.33), precision: 0.001);
    }
    ...
    [InlineData(12.0)]
    [InlineData(18.0)]
    public void Resolve_Superscript_OffsetProportionalToFontSize(double scaledFontSize)
    {
        var style = new CellStyle { Superscript = true };
        CellSuperSubScript.Resolve(style, scaledFontSize, out var adjSize, out var offset);
        adjSize.Should().BeApproximately(scaledFontSize * CellSuperSubScript.FontSizeFactor, precision: 0.001);
        offset.Should().BeApproximately(-(scaledFontSize * CellSuperSubScript.SuperBaselineRatio), precision: 0.001);
    }

    // ── Resolve: subscript ────────────────────────────────────────────────────────────────────────

    [Fact]
    public void Resolve_Subscript_ReducesFontSize()
    {
        var style = new CellStyle { Subscript = true };
        CellSuperSubScript.Resolve(style, 12.0, out var fontSize, out _);
        fontSize.Should().BeApproximately(12.0 * 0.583, precision: 0.001);
    ...
    [Fact]
    public void Resolve_Subscript_PositiveOffset_ShiftsDown()
    {
        var style = new CellStyle { Subscript = true };
        CellSuperSubScript.Resolve(style, 12.0, out _, out var offset);
        // Positive = downward shift; magnitude = fontSize * SubBaselineRatio
        offset.Should().BeApproximately(12.0 * 0.14, precision: 0.001);
    }
    ...
    [InlineData(12.0)]
    [InlineData(18.0)]
    public void Resolve_Subscript_OffsetProportionalToFontSize(double scaledFontSize)
    {
        var style = new CellStyle { Subscript = true };
        CellSuperSubScript.Resolve(style, scaledFontSize, out var adjSize, out var offset);
        adjSize.Should().BeApproximately(scaledFontSize * CellSuperSubScript.FontSizeFactor, precision: 0.001);
        offset.Should().BeApproximately(scaledFontSize * CellSuperSubScript.SubBaselineRatio, precision: 0.001);
    }

    // ── Superscript takes priority over subscript ─────────────────────────────────────────────────

    [Fact]
    public void Resolve_BothSet_SuperscriptTakesPriority()
    {
        var style = new CellStyle { Superscript = true, Subscript = true };
        CellSuperSubScript.Resolve(style, 12.0, out _, out var offset);
        // Superscript → negative (upward) offset wins.
        offset.Should().BeLessThan(0);
    }
}
