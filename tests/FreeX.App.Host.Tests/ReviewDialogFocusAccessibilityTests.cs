    ...
    public void AccessibilityCheckerDialog_WithIssue_FocusesIssueListAndExposesGoToMetadata()
    {
        StaTestRunner.Run(() =>
        {
            ...
            try
            {
                ...
                var statusText = GetField<TextBlock>(dialog, "_statusText");
                var goToButton = GetField<Button>(dialog, "_goToButton");
                var closeButton = GetField<Button>(dialog, "_closeButton");
                var selectedIssue = issueList.SelectedItem.Should().BeOfType<TreeViewItem>().Subject;
                Keyboard.FocusedElement.Should().BeSameAs(selectedIssue);
                selectedIssue.Header.Should().Be("Sheet1!B2");
                statusText.Text.Should().Contain("low contrast");
