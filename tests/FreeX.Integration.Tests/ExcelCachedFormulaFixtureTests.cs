...
public sealed class ExcelCachedFormulaFixtureTests
{
    ...
    [Fact]
    public void LoadedWorkbookRecalc_HonorsExcelSingleImplicitIntersectionFunction()
    {
        using var fixture = CreateLegacyImplicitIntersectionWorkbook("_xlfn.SINGLE(A7:J7)*B15");
        var workbook = new XlsxFileAdapter().Load(fixture);
        var sheet = workbook.Sheets.Should().ContainSingle().Subject;
