...
public sealed partial class MainWindow : Window
{
    ...
    private readonly NativeMenuItem _themesMenuItem = new();
    private readonly NativeMenuItem _themeColorsMenuItem = new();
    ...
    private async Task ShowLegalNoticesDialogAsync()
    {
        ...
        AutomationProperties.SetName(dialog, UiText.Get("LegalNotices_LegalNotices"));
        AutomationProperties.SetAutomationId(dialog, "LegalNoticesDialog");
        AutomationProperties.SetHelpText(dialog, UiText.Get("LegalNotices_ShowsTheLegalPrivacyAndThirdPartyNoticesPackagedWithThisFreeXExecutable"));
        ...
        var closeButton = new Button
        {
            Content = UiText.Get("LegalNotices_CloseButton"),
            IsDefault = true,
            IsCancel = true,
            HorizontalAlignment = AvaloniaHorizontalAlignment.Right,
        };
        ...
        dialog.Opened += (_, _) =>
        {
            if (tabControl.SelectedItem is TabItem { Content: ScrollViewer { Content: TextBox textBox } })
            {
                textBox.Focus();
    ...
    private static TabItem CreateLegalNoticeTabItem(LegalNoticeDocument document)
    {
        var automationIdSegment = CreateLegalNoticeAutomationIdSegment(document.Title);
    ...
    private static string CreateLegalNoticeAutomationIdSegment(string text)
    {
        var chars = text.Where(char.IsLetterOrDigit).ToArray();
        return chars.Length == 0 ? "Document" : new string(chars);
    }
