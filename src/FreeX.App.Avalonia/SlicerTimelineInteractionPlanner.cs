...
public static class SlicerTimelineInteractionPlanner
{
        ...
        DateOnly date)
    {
        ...
        return kind switch
        {
            TimelineHitKind.StartHandle => (date, selectedEnd),
            TimelineHitKind.EndHandle => (selectedStart, date),
            // A track or selection click jumps the whole range to the clicked bucket.
            _ => (date, date),
        };
    }
