...
public sealed class AxisLayout
{
    ...
    /// <summary>
    /// Minor-gridline/tick positions, populated when the chart model requests either minor gridlines or
    /// minor tick marks. Mirrors <see cref="Ticks"/> in shape (unlabeled) so the portable-shell renderers
    /// and the PDF exporter can draw the same geometry as the desktop renderer.
    /// </summary>
