...
public static class GridAutofillPlanner
{
    ...
    public static CellAddress? CalculateDragTarget(
        ...
        double columnHeaderHeight)
    {
        if (!ViewportMetricEndpointLookup.TryFindRows(viewport.RowMetrics, source.Start.Row, source.End.Row, out var srcTopRow, out var srcBottomRow) ||
            !ViewportMetricEndpointLookup.TryFindColumns(viewport.ColMetrics, source.Start.Col, source.End.Col, out var srcLeftCol, out var srcRightCol))
            return null;
