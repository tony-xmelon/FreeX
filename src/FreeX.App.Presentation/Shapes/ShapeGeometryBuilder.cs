...
public static class ShapeGeometryBuilder
{
    ...
    private static ShapeGeometry CylinderShape(LayoutRect rect)
    {
        ...
        var ry = eh / 2;
        var cx = rect.Left + rx;
        var cy = rect.Top + ry;  // center of the top ellipse
        ...
        //
        //   Start at (cx-rx, bodyTop)  ← left edge at ellipse center line
        //   LineTo  (cx-rx, bodyBottom)  ← left side going down
        //   ArcTo   (cx+rx, bodyBottom), rx, ry, clockwise  ← bottom half-ellipse (facing down)
        //   LineTo  (cx+rx, bodyTop)    ← right side going up
        //   ArcTo   (cx-rx, bodyTop),   rx, ry, counterclockwise  ← top half-ellipse (back to start)
