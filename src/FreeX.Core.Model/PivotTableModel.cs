...
/// <summary>
/// Complete allocation-free copy state for <see cref="PivotTableModel"/>. Copy callers apply their
/// intentional identity, range, cache, or package transformations before materializing a new model.
/// </summary>
internal readonly record struct PivotTableCopyState(
    string Name,
    int CacheId,
...
public sealed class PivotTableModel
{
    ...
    internal PivotTableCopyState CaptureCopyState() =>
        new(
            Name,
            CacheId,
            SourceRange,
    ...
    internal static PivotTableModel FromCopyState(PivotTableCopyState state)
    {
        var copy = new PivotTableModel
        {
            Name = state.Name,
            CacheId = state.CacheId,
            SourceRange = state.SourceRange,
            TargetRange = state.TargetRange,
            LastRenderedRange = state.LastRenderedRange,
            PackagePart = state.PackagePart,
            CreatedVersion = state.CreatedVersion,
            UpdatedVersion = state.UpdatedVersion,
            MinRefreshableVersion = state.MinRefreshableVersion,
            DataOnRows = state.DataOnRows,
            FirstHeaderRow = state.FirstHeaderRow,
            FirstDataRow = state.FirstDataRow,
            FirstDataColumn = state.FirstDataColumn,
            ShowSubtotals = state.ShowSubtotals,
            SubtotalPlacement = state.SubtotalPlacement,
            ShowGrandTotals = state.ShowGrandTotals,
            // These assignments intentionally follow ShowGrandTotals. The compatibility alias sets
            // both flags together, while loaded pivots may carry asymmetric row/column settings.
            ShowRowGrandTotals = state.ShowRowGrandTotals,
            ShowColumnGrandTotals = state.ShowColumnGrandTotals,
            RepeatItemLabels = state.RepeatItemLabels,
            BlankLineAfterItems = state.BlankLineAfterItems,
            ReportLayout = state.ReportLayout,
            CompactRowLabelIndent = state.CompactRowLabelIndent,
            ...
            ShowFieldHeaders = state.ShowFieldHeaders,
            ShowContextualTooltips = state.ShowContextualTooltips,
            ShowPropertiesInTooltips = state.ShowPropertiesInTooltips,
            ShowClassicLayout = state.ShowClassicLayout,
            FieldListSortAscending = state.FieldListSortAscending,
            MergeAndCenterLabels = state.MergeAndCenterLabels,
            ShowItemsWithNoDataOnRows = state.ShowItemsWithNoDataOnRows,
            ShowItemsWithNoDataOnColumns = state.ShowItemsWithNoDataOnColumns,
            PageOverThenDown = state.PageOverThenDown,
            PageWrap = state.PageWrap,
            EmptyValueText = state.EmptyValueText,
            ApplyNumberFormats = state.ApplyNumberFormats,
            ApplyBorderFormats = state.ApplyBorderFormats,
            ApplyFontFormats = state.ApplyFontFormats,
            ApplyPatternFormats = state.ApplyPatternFormats,
            AutofitColumnsOnUpdate = state.AutofitColumnsOnUpdate,
            PreserveFormattingOnUpdate = state.PreserveFormattingOnUpdate,
            ShowExpandCollapseButtons = state.ShowExpandCollapseButtons,
            EnableDrill = state.EnableDrill,
            AsteriskTotals = state.AsteriskTotals,
            MultipleFieldFilters = state.MultipleFieldFilters,
            EnableFieldDialog = state.EnableFieldDialog,
            EnableFieldProperties = state.EnableFieldProperties,
            EnableDataValueEditing = state.EnableDataValueEditing,
            PrintTitles = state.PrintTitles,
            PrintExpandCollapseButtons = state.PrintExpandCollapseButtons,
            AltTextTitle = state.AltTextTitle,
            AltTextDescription = state.AltTextDescription,
            DataCaption = state.DataCaption,
            GrandTotalCaption = state.GrandTotalCaption,
            MissingCaption = state.MissingCaption,
            ErrorCaption = state.ErrorCaption
        };
        copy.RowFields.AddRange(state.RowFields);
        copy.ColumnFields.AddRange(state.ColumnFields);
        copy.PageFields.AddRange(state.PageFields);
        copy.DataFields.AddRange(state.DataFields);
        copy.CalculatedFields.AddRange(state.CalculatedFields);
        copy.CalculatedItems.AddRange(state.CalculatedItems);
        copy.LabelFilters.AddRange(state.LabelFilters);
        copy.ValueFilters.AddRange(state.ValueFilters);
        copy.Sorts.AddRange(state.Sorts);
        return copy;
    }
}
