...
public sealed record WorkbookExportReadinessPlan(
    bool IsReady,
    string StatusText);

public static class WorkbookExportReadinessPlanner
{
    ...
    public static WorkbookExportReadinessPlan Create(Workbook workbook, bool hasSelection = false)
    {
        ArgumentNullException.ThrowIfNull(workbook);

        if (!HasVisibleWorksheet(workbook))
        {
            return new WorkbookExportReadinessPlan(
                false,
    ...
    public static WorkbookExportReadinessPlan CreateForAvailableWorkbook(bool hasSelection = false)
    {
