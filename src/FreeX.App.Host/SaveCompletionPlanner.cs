...
/// <summary>
/// Pure decision logic for the post-save completion step.
...
/// Extracted for unit-testability without a live WPF window.
/// </summary>
public static class SaveCompletionPlanner
{
    ...
    ///   The <c>_workbookDirtyGeneration</c> value captured just before the async save awaited.
    /// </param>
    /// <param name="generationNow">
    ///   The current <c>_workbookDirtyGeneration</c> value after the await completes.
    /// </param>
...
///   When <c>true</c>, call <c>MarkWorkbookSaved()</c> to clear the dirty flag.
///   When <c>false</c>, the workbook has unsaved edits that arrived after the save started.
/// </param>
/// <param name="ApplyFileContext">
///   When <c>true</c>, apply the post-save file context: update <c>_currentFilePath</c>,
///   <c>_workbook.Name</c>, and the recent-files list.
