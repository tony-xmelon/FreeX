            ...
            IReadOnlyList<CellTextRun> runs)
        {
            ...
            foreach (var run in runs)
            {
                    ...
                    run.VertAlign != CellTextRunVertAlign.None)
                {
                    ...
                    // Order follows OOXML CT_RPrElt schema: b, i, strike, u, vertAlign, sz, color, rFont, …
                    if (run.Bold is { } b)
                    {
                    ...
                    if (run.FontSize is { } sz)
                        rPr.Add(new XElement(worksheetNs + "sz",
                            new XAttribute("val", sz.ToString(System.Globalization.CultureInfo.InvariantCulture))));

                    if (run.FontColor is { } color)
                        rPr.Add(new XElement(worksheetNs + "color",
                            new XAttribute("rgb",
