...
internal static class XlsxAllowEditRangeMapper
{
    ...
    private static bool TryParseSqrefToken(string token, SheetId sheet, out GridRange range)
    {
        ...
        // FreeX, e.g. Parser.cs/FormulaEvaluator.References.cs using CellAddress.MaxRow/MaxCol) so a
        // protected sheet's "Allow Users to Edit Ranges" entry for an entire column/row is actually
