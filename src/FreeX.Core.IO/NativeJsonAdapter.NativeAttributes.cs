...
public sealed partial class NativeJsonAdapter
{
    ...
    private static Dictionary<string, string>? CleanOptionalNativeAttributes(IReadOnlyDictionary<string, string>? attributes)
    {
        var cleaned = CleanNativeAttributesCore(attributes);
        return cleaned.Count == 0 ? null : cleaned;
    }

    private static Dictionary<string, string> CleanNativeAttributesCore(IReadOnlyDictionary<string, string>? attributes) =>
