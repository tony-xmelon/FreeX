...
2. **Package-preserving XLSX save path**
   ...
   - **Excel-openability of FreeX-authored XLSX — was broken, now substantially fixed (2026-06-01).** The never-run "manual desktop Excel open/save/reopen" check turned up that FreeX output did not open in Microsoft Excel at all. Two root causes were found (with the Open XML SDK validator + real Excel via COM) and fixed: (a) the modeled `theme1.xml` was schema-invalid (incomplete `fontScheme`/`fmtScheme`), blocking **every** workbook; (b) chart parts were schema-invalid (classic title/axis `<a:bodyPr>` missing, `<c:lineChart>` missing `<c:grouping>`, worksheet `<drawing>` mis-ordered before `tableParts`, and the chartEx `cx:` title/legend/dataId/binning shapes), blocking **any** workbook with a chart. Plain workbooks and **classic charts (Column/Bar/Line/Pie/Area/Scatter) now open in Excel — verified**. A permanent `XlsxSchemaValidationTests` gate (Open XML SDK, Excel-independent) now guards all chart types' schema validity.
   - **Chart interop status improved 2026-06-01.** Modern chartEx openability moved from a broad blocker into evidence-driven hardening. The latest complete chart comparison run passed 28/28 chart cases, including chartEx families. Recent fixes include treemap/sunburst size dimensions, histogram default binning, Pareto aggregation/axis metadata, box-and-whisker series and axis metadata, waterfall chartEx XML, and focused branch-head reruns for Pareto, 3-D bar, and box-and-whisker. Remaining chart work is visual/layout fidelity, per-family edge cases, and keeping the chart comparison evidence green as writer changes continue.
...
2. **Charts, themes, and visual objects**
   - Chart interop comparison harness is active and current. It compares FreeX renderer PNGs, FreeX-authored XLSX opened/exported by Excel, Excel-authored XLSX opened/exported by Excel, and Excel-authored XLSX loaded/saved by FreeX then reopened/exported by Excel. Latest complete run: 28/28 chart cases passed openability/export and visual gates; later repeated Excel COM diagnostics are tracked as automation/openability failures rather than visual mismatches.
   ...
   - Waterfall "Set as Total" per-point totals — **modeled + rendered + native-persisted + UI-wired 2026-06-01** (pure `WaterfallBarPlanner` with 11 unit tests, `ChartModel.WaterfallTotalPointIndices`, renderer delegates to the planner, plus the Set as Total UI path). This also fixed a rendering bug where a total column was drawn from the running total up by its own value (approximately `[sum, 2*sum]`) instead of as an anchor from zero to the cumulative (`[0, sum]`). Follow-on: XLSX read/write of the total-point flags and visual styling refinements.
   - Full chart format panes/dialog UX.
   - Richer combo-chart mixes and advanced chart families such as treemap, sunburst, histogram, Pareto, box-and-whisker, waterfall, funnel, map, and true 3D mesh-style surface polish; blank-display rendering now covers line/area plus blank-as-zero column/bar charts, 2D/3D surface charts have standard OOXML package parts with series axes and value-colored matrix rendering paths, 3D clustered column/bar, 3D line, 3D area, and 3D pie now have standard OOXML package/rendering paths, Pareto/box-and-whisker/waterfall chartEx interop has current Excel-openability evidence, and stock chart parity now includes high-low-close, open-high-low-close, volume stock package/rendering paths, date-axis rendering, and up/down bar candlestick rendering but still needs deeper formatting preset polish.
