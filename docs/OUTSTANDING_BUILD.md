...
2. **Package-preserving XLSX save path**
   ...
   - **Remaining for modern (chartEx) charts to open in Excel:** the `cx:` parts are now schema-valid but Excel additionally requires the modern-chart package extras that FreeX does not yet emit — `xl/charts/colorsN.xml` (`chartcolorstyle+xml`) and `xl/charts/styleN.xml` (`chartstyle+xml`) parts, their `[Content_Types]` overrides and `chartExN.xml.rels` relationships, and an `mc:AlternateContent`/`mc:Fallback` wrapper around the drawing `graphicFrame` (Excel authors a `twoCellAnchor` with a fallback shape). Until then histogram/waterfall/treemap/sunburst/Pareto/funnel/box-&-whisker charts still will not open in Excel even though they round-trip through FreeX and validate against the schema.
...
2. **Charts, themes, and visual objects**
   ...
   - Waterfall "Set as Total" per-point totals — **modeled + rendered + native-persisted 2026-06-01** (pure `WaterfallBarPlanner` with 11 unit tests, `ChartModel.WaterfallTotalPointIndices`, renderer delegates to the planner). This also fixed a rendering bug where a total column was drawn from the running total up by its own value (≈ `[sum, 2·sum]`) instead of as an anchor from zero to the cumulative (`[0, sum]`). Follow-on: a "Set as Total" context action/dialog and XLSX read/write of the total-point flags.
