...
non-General number-format cells. Structure tags assert Excel-visible merged areas, freeze panes,
hidden rows/columns, custom row/column dimensions, and outline rows/columns. Page setup tags now
...
drawing/shape, sparkline/image, formatting/style/number-format, structure, protection/page-setup,
allow-edit-range, and PivotTable feature
...
pictures, sparklines, text boxes, drawing shapes, Excel-visible formatting/number-format,
...
- Excel-side formatting assertions now cover generated supported-pass formatting/style rows through
  Excel open/reopen: styled cells and non-General number-format cells are asserted where tagged. A
  focused 2026-06-04 generated `supported-pass` run passed `52/52` after FreeX resave with those
  counters enabled, and caught a built-in Excel number-format ID load regression before the fix.
