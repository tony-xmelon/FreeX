...
| Non-finite ENTERING the model from a parsed file/clipboard | r547, r548, r549 | FIXED at 6 FreeW reader sites: double.TryParse returns true with Infinity on overflow (1e400), so no hostile token is needed. NumberStyles.Integer sites cannot; Math.Clamp bounds Infinity but passes NaN. FreeX/FreeP readers not yet swept FreeP swept exhaustively in r548 (15 of 17 sites unguarded; r486 had fixed 2). FreeX's 77 still unswept FreeX swept in r549 (13 of 77 fixed; 21 already guarded). Three disguises recorded: Math.Clamp passes NaN, comparison guards fail against NaN, and rot % 360 MANUFACTURES NaN from Infinity. IO layers only -- app/presentation layers unswept |
