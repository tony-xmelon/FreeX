...
- ✅ **DONE 2026-07-01 (`5dcbe733c`, `e559ece6a`)** - Remaining FreeX tooling harness helpers. `FreeX.ExcelExamplesCharts` now uses `WpfImageDiff`, `WpfSideBySidePng.WriteHeaderOnly`, `ExcelComAutomation.CreateExcelApplicationWithRetry`, `GetNewExcelProcessIds`, `KillExcelProcesses`, and `ToolFileNameSanitizer`. `FreeX.FormatFidelity` and `FreeX.FormatCrossCheck` use the shared sanitizer. `FreeX.FidelityCompare` and `FreeX.ForegroundCapture` now use shared Excel COM creation/process helpers while keeping their scenario-specific workbook behavior local. `ToolHarnessDedupSourceTests` guards the helper adoption.
