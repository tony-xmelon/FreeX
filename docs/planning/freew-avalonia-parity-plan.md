...
Status 2026-07-04: the visual evidence summary now reports the references-heavy TOA page-number blocker as machine-checkable data. `freew_visual_evidence_summary.json` schema v16 includes `remainingEvidenceBlockers`; when `references-heavy-fields` has trusted WPF/Avalonia evidence but Word COM/Word PNG baselines are unavailable or missing, the summary emits `references-heavy-toa-page-number-fidelity` with the related baseline statuses and candidate baseline paths. This keeps no-Word runs trusted for renderer evidence while preserving the explicit blocker instead of implying Microsoft Word visual parity.
