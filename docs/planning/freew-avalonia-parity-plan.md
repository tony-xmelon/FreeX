...
The next parity work should therefore avoid command-count chasing. Remaining value is in deeper proof: Word-baseline visual comparison, true Avalonia mixed-section page surfaces, broader fixture coverage beyond the current table/floating object/chart/SmartArt/WordArt evidence set, and behavior evidence where a command exists but Word-like results are still only weakly proven.
