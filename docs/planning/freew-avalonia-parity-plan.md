...
Status 2026-07-13: nested OfficeMath work has started through narrow shared slots. `d24964d11` adds nested equation numerator/denominator slots for fractions, `91de41490` adds nested radical radicands while keeping radical degree text-only, `19568dc8b` adds nested delimiter content, `8b7fe1b3a` adds nested function arguments, `69460408b` adds nested n-ary lower/upper/operand slots, `592b5e055` adds nested base/subscript/superscript slots for `m:sSub`, `m:sSup`, and `m:sSubSup`, and `daf281698` adds nested decorator base/content slots for accent, bar, and group-char OMML structures. The shared model, DOCX reader/writer, and equation visual planner own those slots; WPF and Avalonia continue to consume the shared flattened segment contract plus the nested slot plans without host-specific equation engines.
...
Status 2026-07-07: comments-only protection/proofing evidence is now part of the shared visual-evidence contract through `e84689909`. The `review-protection-proofing-depth` fixture records CommentsOnly protection, Restrict Editing checked state, Mark as Final unchecked state, proofing diagnostic preservation, blocked body-edit/body-format/proofing-replace/history commands, and allowed classified comment workflows. WPF `FreeW.FidelityRender` and Avalonia `FreeW.PageLayoutShot` emit the shared manifest facts through thin evidence-tool paths. MarkedAsFinal interactions and raw proofing pixel equivalence remain separate work.
...
Status 2026-07-07: comments-only protection/proofing evidence is refreshed through the shared visual manifest (`e84689909`). WPF and Avalonia now report the same CommentsOnly fixture facts for protection state, proofing diagnostics, blocked body/proofing commands, blocked unknown history, and allowed classified comment workflows. The remaining review limitation is no longer absence of shared evidence for CommentsOnly; it is WPF native RichTextBox history introspection, MarkedAsFinal interactions, retained-model safety, and any raw proofing-pixel differences.
