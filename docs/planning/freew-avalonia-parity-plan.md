...
Status 2026-07-03: cross-reference Update Fields behavior is now shared through `FreeW.Core.Model.CrossReferences` and consumed by both WPF and Avalonia document views. Source entry is also type-aware through the shared source-management planner for the three modeled source types. Remaining references depth should focus on table-of-authorities workflow polish, citation-style breadth/formatting polish, broader source-manager behavior beyond the document-local modeled fields, and real visual/round-trip evidence for references-heavy documents rather than reimplementing cross-reference refresh or source-entry field policy in host code.
