...
## Current July 3 Integrated Update

The command-surface phase is now exhausted for actionable WPF/Avalonia deltas. The generated FreeW command inventory reports `798` total commands with `0` actionable WPF gaps, `0` actionable Avalonia gaps, and `0` total actionable gaps. Raw `wpfOnly` / `avaloniaOnly` counts still include profile-shape, alias, deferred, and platform-specific rows; they are not implementation targets by themselves.
