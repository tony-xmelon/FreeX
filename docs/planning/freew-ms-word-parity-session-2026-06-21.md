...
July 2 also landed the last ready Insert > Links functional salvage for Avalonia hyperlink/bookmark actions and expanded the shared WPF/Avalonia visual evidence contract. WPF remains the Microsoft Word behavior oracle, but both WPF `FreeW.FidelityRender` and Avalonia `FreeW.PageLayoutShot` now emit a shared visual manifest for F2/page-composition scenarios through `FreeW.App.Presentation` planners and trust checks. The current no-Word smoke path generates 15 fixtures, including footnote/endnote placement, section geometry, table layout, drawing objects, and chart/SmartArt composition coverage; WPF renders 32 PNGs, Avalonia renders 20 PNGs, and `FreeW.VisualEvidenceSummary` reports 52 trusted rows. WPF emits true mixed portrait/landscape `f2-section-landscape` dimensions; Avalonia emits the shared section ownership and expected page-geometry metadata but still marks its actual rendering as a global page-surface/no-section-page-break gap.
