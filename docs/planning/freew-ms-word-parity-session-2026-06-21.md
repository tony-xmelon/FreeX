...
4. Close References/source-management/table-of-authorities gaps through `FreeW.Core.Model`, `FreeW.Core.IO`, and `FreeW.App.Presentation` before renderer work. Cross-reference refresh, type-aware source entry, and Mark Citation category/short-citation entry are now shared-first; next references work should target citation formatting/source-manager breadth, TOA output/evidence polish, or real references-heavy visual/round-trip proof.
