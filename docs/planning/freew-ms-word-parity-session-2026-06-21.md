...
July 2 also landed the last ready Insert > Links functional salvage for Avalonia hyperlink/bookmark actions and expanded the shared WPF/Avalonia visual evidence contract. WPF remains the Microsoft Word behavior oracle, but both WPF `FreeW.FidelityRender` and Avalonia `FreeW.PageLayoutShot` now emit a shared visual manifest for F2/page-composition scenarios through `FreeW.App.Presentation` planners and trust checks. The current no-Word smoke path generates 15 fixtures, including table layout, drawing objects, and chart/SmartArt composition coverage; WPF renders 32 PNGs, Avalonia renders 13 PNGs, and `FreeW.VisualEvidenceSummary` reports 45 trusted rows.
...
The next Word-parity evidence work is no longer command exposure; it is stronger visual/behavior proof, especially Word-baseline pixel comparison, tolerance reporting, and broader fixture coverage for remaining open-ended polish cases such as WordArt, watermark/layout stress, footnote/endnote page placement, and section geometry.
