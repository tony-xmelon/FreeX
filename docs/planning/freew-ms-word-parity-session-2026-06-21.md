...
4. Close References/source-management/table-of-authorities gaps through `FreeW.Core.Model`, `FreeW.Core.IO`, and `FreeW.App.Presentation` before renderer work. Cross-reference refresh, type-aware source entry, Mark Citation category/short-citation entry, in-text personal-author display, tagged `CITATION` field refresh, and bibliography-region planning are now shared-first; next references work should target source-manager breadth, full structured bibliography fidelity, TOA page-number/evidence polish, or real references-heavy visual/round-trip proof.
