...
July 4 numeric citation update: the shared `Citations` model now resolves IEEE/Vancouver in-text citations from the document source list so inserted citations use stable source-order numbers such as `[1]` and `[2]`, and repeated citations of the same source/tag reuse the same number. Numeric bibliography/reference-list generation now keeps source order and prefixes each entry with its numeric reference marker. WPF and Avalonia document views remain thin consumers of the shared formatter. Remaining limitations: inserted citations and generated reference lists are still ordinary visible text, not live Word `CITATION` / `BIBLIOGRAPHY` field runs; existing inserted text is not automatically renumbered after source-list edits; real Word visual baselines still need Word COM on the validation machine.
...
4. Close References/source-management/table-of-authorities gaps through `FreeW.Core.Model`, `FreeW.Core.IO`, and `FreeW.App.Presentation` before renderer work. Cross-reference refresh, type-aware source entry, Mark Citation category/short-citation entry, and in-text personal-author display are now shared-first; next references work should target source-manager breadth, citation formatting beyond family-name display, TOA output/evidence polish, or real references-heavy visual/round-trip proof.
