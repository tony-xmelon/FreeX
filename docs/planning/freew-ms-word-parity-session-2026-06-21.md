...
July 2 also landed the last ready Insert > Links functional salvage for Avalonia hyperlink/bookmark actions and expanded the shared WPF/Avalonia visual evidence contract. WPF remains the Microsoft Word behavior oracle, but both WPF `FreeW.FidelityRender` and Avalonia `FreeW.PageLayoutShot` now emit a shared visual manifest for F2/page-composition scenarios through `FreeW.App.Presentation` planners and trust checks. The current no-Word smoke path generates 17 fixtures, including footnote/endnote placement, section geometry, table layout, drawing objects, chart/SmartArt composition, WordArt-over-watermark stress, and WordArt plus picture-watermark layout stress coverage; WPF renders 35 PNGs, Avalonia renders 22 PNGs, and `FreeW.VisualEvidenceSummary` reports 57 trusted rows. WPF emits true mixed portrait/landscape `f2-section-landscape` dimensions; Avalonia now renders that scenario through the shared section-surface plan as separate portrait and landscape page-surface captures with matching section ownership metadata.

Latest July 2 evidence-path update: Word-baseline comparison reporting is now explicit enough for triage. Summary JSON/Markdown rows carry a baseline id, mapped baseline scenario, candidate baseline path(s), comparison status, skip reason, tolerance limits, and changed-pixel metrics when PNGs are compared. When Word COM is unavailable, `tools/Run-FreeWWordBaselineEvidence.ps1 -AllowMissingWord` records `word-baseline-unavailable` rows and keeps trust green; a forced no-Word validation produced 57 comparison rows with `skipped=4, word-baseline-unavailable=53`.
