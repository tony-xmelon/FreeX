...
Nested OfficeMath work has started through narrow shared slots rather than a broad recursive rewrite. `d24964d11` adds nested numerator/denominator slots for fractions, and `91de41490` adds nested radical radicand slots while keeping radical degree text-only. In both cases DOCX read/write emits child math runs directly in the relevant OMML slot, and the shared equation visual planner exposes nested slot plans while preserving the existing flattened segment contract for WPF and Avalonia. Focused validation reported by the workers covered equation model, IO round-trip, visual planner, and serialized `FreeW.slnx` Release builds with 0 warnings/errors. Remaining equation work is now nested slots beyond fraction numerator/denominator and radical radicand, richer Word baseline comparison, and pixel-level equation geometry/spacing, not another per-`MathRunKind` WPF/Avalonia host split.
...
4. Treat remaining equation work as nested OfficeMath slots beyond fractions and radical radicands, Word-baseline comparison, and geometry/spacing fidelity. The July 7 shared equation planner closes the current modeled-kind fallback layer for both WPF and Avalonia, `d24964d11` adds nested fraction slots, and `91de41490` adds nested radical radicands.
