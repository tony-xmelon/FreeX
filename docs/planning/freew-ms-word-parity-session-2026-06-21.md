...
The next Word-parity evidence work is no longer command exposure; it is stronger visual/behavior proof, especially evaluating real Word PNG comparisons from the strengthened baseline/tolerance report on a Word-capable machine, broadening fixture coverage for remaining open-ended polish cases beyond the current mixed-section, table-cell-border, protection-history, cross-reference-refresh, drawing-effects, WordArt/watermark, run-decoration, visible note-region, object-format, and references-heavy fixtures, and adding behavior evidence for high-risk shared-profile commands where Word-like results are still weakly proven. This is not a claim of full MS Word visual parity.
...
4. Close References/source-management/table-of-authorities gaps through `FreeW.Core.Model`, `FreeW.Core.IO`, and `FreeW.App.Presentation` before renderer work. Cross-reference refresh, type-aware source entry, structured source-author persistence for modeled Book/Journal/Web sources, Mark Citation category/short-citation entry, in-text personal-author display, tagged `CITATION` field refresh, and bibliography-region planning are now shared-first; next references work should target broader source-manager catalogs/master-source semantics, remaining structured bibliography fields beyond people/corporate authors, TOA page-number/evidence polish, or real references-heavy visual/round-trip proof.
