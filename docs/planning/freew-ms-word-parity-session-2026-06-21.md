...
The first nested OfficeMath slice is integrated on `origin/main` at `d24964d11`. `MathRun` now supports nested equation numerator/denominator slots for fractions, DOCX read/write emits those child math runs directly under `m:num` / `m:den`, and the shared equation visual planner exposes nested fraction slot plans while preserving the existing flattened segment contract for WPF and Avalonia. Focused validation reported by the worker covered `EquationRoundTripTests` 21/21, `EquationVisualPlannerTests` 16/16, `EquationsTests` 13/13, and a serialized `FreeW.slnx` Release build with 0 warnings/errors. Remaining equation work is now broader nested slots beyond fractions, richer Word baseline comparison, and pixel-level equation geometry/spacing, not another per-`MathRunKind` WPF/Avalonia host split.
...
4. Treat remaining equation work as nested OfficeMath slots beyond fractions, Word-baseline comparison, and geometry/spacing fidelity. The July 7 shared equation planner closes the current modeled-kind fallback layer for both WPF and Avalonia, and `d24964d11` adds the first nested fraction-slot path.
