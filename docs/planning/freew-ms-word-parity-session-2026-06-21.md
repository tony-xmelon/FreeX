...
Latest July 3 evidence-path update: Word-baseline comparison reporting is explicit enough for triage. Summary JSON/Markdown rows carry a baseline id, mapped baseline scenario, candidate baseline path(s), comparison status, skip reason, tolerance limits, and changed-pixel metrics when PNGs are compared. On this machine Word COM remains unavailable, so `tools/Run-FreeWWordBaselineEvidence.ps1 -AllowMissingWord` generated no real Word PNGs; the integrated run rendered 18 DOCX fixtures / 28 WPF outputs through software fallback and reported 54 trusted evidence rows plus 54 baseline comparisons with `skipped=8, word-baseline-unavailable=46`.
