...
July 3 drawing-effects evidence update: `origin/main` contains `006988bfe` for the shared drawing-effect evidence contract. The normalizer now treats `drawing-objects-complex` and `wordart-watermark-stress` as expected drawing-object renderer scenarios, and the shared summary/manifest contract proves top-level effect-bearing objects in both WPF and Avalonia evidence: `drawing-objects-complex` has 3 effect objects (shape shadow, image shadow/glow/reflection/artistic effect, WordArt glow), while `wordart-watermark-stress` has 2 (shape shadow, WordArt glow). The deliberate limitation is grouped-child evidence: `DrawingObjectVisualPlanner` still plans grouped child effects, but WPF DOCX evidence and Avalonia direct-fixture evidence do not surface those grouped child effects identically, so host evidence summaries do not claim them yet.

July 3 grouped-child drawing-effect evidence update: the shared visual-evidence schema now exposes grouped-child effect plans separately from top-level rendered effect claims. The `drawing-objects-complex` fixture records 3 claimed top-level effect objects plus 1 planned grouped child effect (`GroupChild0:Shape:glow`). This keeps WPF/Avalonia rich-rendering triage honest: grouped child effects are now visible as an exact remaining evidence/rendering gap instead of being hidden inside the group plan.
