...
Latest July 3 evidence-path update: Word-baseline comparison reporting is explicit enough for triage. Summary JSON/Markdown rows carry a baseline id, mapped baseline scenario, candidate baseline path(s), comparison status, skip reason, tolerance limits, and changed-pixel metrics when PNGs are compared. On this machine Word COM remains unavailable, so `tools/Run-FreeWWordBaselineEvidence.ps1 -AllowMissingWord` generated no real Word PNGs; the latest local run rendered 19 DOCX fixtures / 31 WPF outputs through software fallback and reported 60 trusted evidence rows plus 60 baseline comparisons with `skipped=14, word-baseline-unavailable=46`. The new field page-number visual evidence scenario is integrated on `origin/main` (`11e7c74ee`, worker `ddba30497`) and accounts for the expanded fixture/output/evidence totals.
