...
The July 7 Source Manager breadth tranche is also integrated on `origin/main`: `ce11e085f` adds Patent, Interview, and Misc; `e61d1272e` adds Film, Sound Recording, Art, Internet Site, and Performance; and `bba3e7018` adds bibliography Case without conflating it with the separate Table of Authorities `Citation` model. The shared model/IO/planner path now covers the current Word/OOXML bibliography source-type set FreeW targets, with WPF and Avalonia dialogs remaining thin consumers. Focused validation reported by the workers covered source-management planner tests, bibliography round-trip tests, citation formatting tests, master-source persistence tests, WPF/Avalonia source guards, and `FreeW.slnx` Release builds. Remaining references work is no longer basic source-type breadth; it is full Word structured bibliography XML/content-control fidelity, richer master-source-library semantics, live TOA page-number fidelity, and real references-heavy visual/round-trip proof.

Nested OfficeMath work has started through narrow shared slots rather than a broad recursive rewrite. `d24964d11` adds nested numerator/denominator slots for fractions, `91de41490` adds nested radical radicand slots while keeping radical degree text-only, `19568dc8b` adds nested delimiter content slots, and `8b7fe1b3a` adds nested function-argument slots. In all four cases DOCX read/write emits child math runs directly in the relevant OMML slot, and the shared equation visual planner exposes nested slot plans while preserving the existing flattened segment contract for WPF and Avalonia. Focused validation reported by the workers covered equation model, IO round-trip, visual planner, WPF/Avalonia source guards where relevant, and serialized `FreeW.slnx` Release builds with 0 warnings/errors. Remaining equation work is now nested slots beyond fraction numerator/denominator, radical radicand, delimiter content, and function arguments; richer Word baseline comparison; and pixel-level equation geometry/spacing, not another per-`MathRunKind` WPF/Avalonia host split.

The next Word-parity evidence work is no longer command exposure, basic source-type breadth, or modeled-structure equation fallback coverage. It is stronger visual/behavior proof: evaluate real Word PNG comparisons from the strengthened baseline/tolerance report on a Word-capable machine, broaden fixture coverage beyond the current mixed-section, table-cell-border, protection-history, cross-reference-refresh, drawing-effects, WordArt/watermark, run-decoration, visible note-region, object-format, references-heavy, and equation-structures fixtures, and add behavior evidence for high-risk shared-profile commands where Word-like results are still weakly proven. This is not a claim of full MS Word visual parity.
...
2. Use the shared visual evidence loop for pagination, tables, floating objects, headers/footers, charts, SmartArt, WordArt/watermark, references-heavy fields, and equations.
...
4. Treat remaining equation work as nested OfficeMath slots beyond fractions, radical radicands, delimiter content, and function arguments; Word-baseline comparison; and geometry/spacing fidelity. The July 7 shared equation planner closes the current modeled-kind fallback layer for both WPF and Avalonia, `d24964d11` adds nested fraction slots, `91de41490` adds nested radical radicands, `19568dc8b` adds nested delimiter content, and `8b7fe1b3a` adds nested function arguments.
5. Keep references work shared-first: full Word structured bibliography XML/content-control fidelity, richer master-source-library semantics, TOA page-number/evidence polish, and real references-heavy visual/round-trip proof.
6. Keep Review and View work shared-first: proofing/protection/tracked-change evidence, retained-model safety, read/split/window behavior, and renderer limitations such as true editable multi-page/side-to-side layouts.
