...
July 2 also landed the last ready Insert > Links functional salvage for Avalonia hyperlink/bookmark actions and expanded the shared WPF/Avalonia visual evidence contract. WPF remains the Microsoft Word behavior oracle, but both WPF `FreeW.FidelityRender` and Avalonia `FreeW.PageLayoutShot` now emit a shared visual manifest for F2/page-composition scenarios through `FreeW.App.Presentation` planners and trust checks. The current no-Word smoke path generates 16 fixtures, including footnote/endnote placement, section geometry, table layout, drawing objects, chart/SmartArt composition, and WordArt-over-watermark stress coverage; WPF renders 33 PNGs, Avalonia renders 21 PNGs, and `FreeW.VisualEvidenceSummary` reports 54 trusted rows. WPF emits true mixed portrait/landscape `f2-section-landscape` dimensions; Avalonia emits the shared section ownership and expected page-geometry metadata but still marks its actual rendering as a global page-surface/no-section-page-break gap.
...
The next Word-parity evidence work is no longer command exposure; it is stronger visual/behavior proof, especially Word-baseline pixel comparison, tolerance reporting, true Avalonia mixed-section page surfaces, and broader fixture coverage for remaining open-ended polish cases beyond the current WordArt/watermark stress fixture.
