...
Branch/worktree context for this update: `codex/freep-parity-report-transition-retention-refresh-20260701` at `origin/main` commit `45e4afa16`. The update is docs-only and is based on local inspection plus the 2026-07-01 FreeP parity implementation wave.

## Current Evidence

- Primary repo root: `C:\Users\anton\OneDrive\Documents\FreeX\FreeX`.
- `main` was clean and synced with `origin/main` at `45e4afa16` before this report branch was created.
...
The first post-dedup FreeP parity implementation wave is integrated on `main` through `45e4afa16`. It closed the initial evidence and shared-planning gates that were blocking reliable WPF/Avalonia parity work:
...
- Expanded the Avalonia ribbon profile from the initial small surface to 59 shared command ids. The generated command inventory currently reports 94 total commands, 59 shared, 0 explicit Avalonia gaps, 27 known deferred commands, 6 platform-only commands, and 2 command-id aliases.
...
- Added semantic-edit package-retention coverage for high-risk corpus decks with media, charts, SmartArt diagrams, picture crop, chart types, and chart labels.
...
| Avalonia app | `freep/FreeP.App.Avalonia/**`, `freep/FreeP.App.Rendering.Avalonia/**`, `freep/FreeP.App.Avalonia.Tests/**`, `freep/FreeP.App.Rendering.Avalonia.Tests/**`. | Cross-platform shell and renderer exist. Avalonia consumes shared file workflow, ribbon definitions, slide pane planning, rendering planners, shared presenter state, shared export planning, and shared transition commands. No explicit Avalonia command gaps remain in the generated inventory; larger Design/Animations slices are intentionally classified as deferred. |
...
| Package retention | `freep/FreeP.Core.Model/PptxPackageSnapshot.cs`, `PptxPackageReader.cs`, `PptxPackageWriter.cs`, `freep/FreeP.App.Host.Tests/PptxPackageRetentionTests.cs`. | Preservation machinery exists, the 20 tracked corpus decks have a retention contract test, and six high-risk corpus decks now have semantic modeled-shape edit/save/reopen retention coverage. It still needs comments/notes fixtures and broader semantic edit breadth. |
...
| --- | --- | --- | --- | --- |
| FreeP WPF-vs-Avalonia command matrix | The generated matrix now exists and is the source of truth for command-surface work. Current state: 94 total, 59 shared, 0 explicit Avalonia gaps, 27 known deferred, 6 platform-only, 2 command-id aliases. | Keep `tools/Generate-FreePCommandParityInventory.ps1` and generated docs current as command slices land. | WPF/Avalonia only classify adapter-only or platform-only commands. | `tools/Test-GeneratedDocs.ps1`; `FreePRibbonDefinitionProfileTests.cs`. |
| Ribbon depth and stubs | The explicit Avalonia command gaps are closed, and transition command IDs are exposed through WPF/Avalonia planner-backed routes. Design, Animations, Review/View/Accessibility/Recording, and layout controls remain incomplete or deferred. | Shared ribbon definitions, command planners, layout/timing/animation models in `FreeP.App.Presentation`. | Host adapters render galleries, combo boxes, and dialogs. | Command inventory deltas; `RibbonEditorCompleteness5BTests.cs`; `RibbonTransitionsAnimationsTests.cs`; new stub-failure tests. |
...
| Package fidelity corpus contract | The 20 tracked decks now have a package-retention contract, and media/chart/SmartArt-focused decks have semantic modeled-shape edit coverage. Coverage is still thin for embedded workbooks, notes, comments, custom XML, and view/print settings. | `FreeP.Core.IO` package classification and writer merge rules; shared OPC helpers stay generic. | Hosts do not own package semantics. | Expand corpus open-save-reopen tests with targeted ZIP/OpenXML assertions after writer-owned edits. |
...
2. Done for explicit gaps: expand Avalonia ribbon profile so Home clipboard/font/arrange/editing/slides, Insert shapes/tables/charts/links, and Transitions are shared. Design and Animations remain known-deferred slices.
...
Current integration stance: FreeP has a substantial shared codebase, generated command inventory with zero explicit Avalonia command gaps, 20-deck package-retention contracts plus first semantic-edit corpus coverage, trustworthy render-harness failure reporting, initial shared chart/text/gesture planners, shared presenter/slideshow state consumed by WPF and Avalonia, shared export/backstage planning with WPF/Avalonia PDF export routes, and shared transition command intents exposed through WPF/Avalonia. It is not PowerPoint-parity complete. The next work should generate missing PowerPoint refs on a COM-capable machine, then continue deeper shared planner work for Design/Animations, text, charts, export/print depth, presenter recording/ink tools, slide-pane/table editing, comments/notes corpus fixtures, and review/accessibility surfaces.
