...
Branch/worktree context for this update: `codex/freep-parity-report-live-refresh-20260701`, based on `origin/main` commit `cc66c5e20`. This update reflects the 2026-07-01 FreeP parity implementation wave through the integrated table-cell editing planner, Avalonia table-cell overlay, presenter recording/ink intent, image export execution, review workflow adapter, text placeholder fidelity, and image export UI-flow slices.
...
The first post-dedup FreeP parity implementation wave is integrated on `origin/main` through `cc66c5e20`, including `bc1d49b57` shared table-cell planning, `3451f903f` Avalonia table-cell overlay wiring, `322ada635` presenter recording/ink intents, `267cc43b2` shared image export execution, `12f5e7571` review workflow host adapters, `2cf014219` text placeholder inset inheritance, and `cc66c5e20` image export UI flow. It closed the initial evidence and shared-planning gates that were blocking reliable WPF/Avalonia parity work:
...
- Added thin WPF and Avalonia review workflow adapters that expose shared comments-pane, accessibility-summary, alt-text request, and proofing plans through host command/state seams. Modern resolved-thread state, persistent alt-text model/IO, full proofing execution, and full accessibility checking remain deferred.
...
| Comments, accessibility, proofing, and alt text | PowerPoint includes modern comments, accessibility/alt text workflows, proofing, and review surfaces. FreeP now has a shared planner boundary for legacy comment pane actions, add/edit/delete intent payloads, accessibility issue summaries, alt-text request descriptors, and proofing scopes, plus thin WPF/Avalonia adapters that expose the shared comments-pane, accessibility-summary, alt-text request, and proofing plans as host state/commands. It still lacks modern resolved-thread state, persistent alt-text model/IO, a full proofing engine, a full accessibility checker, and complete review UI parity. | `PresentationReviewWorkflowPlanner`, comments/review/accessibility planners, model/IO for comments and alt text. | Pane/dialog realization, focus/UIA, and host command/state adapters. | `PresentationReviewWorkflowPlannerTests.cs`; `ReviewWorkflowAdapterTests.cs`; `MainWindowHeadlessTests.cs`; `SectionsCommentsTests.cs`; new modern comments/accessibility/alt-text UI tests. |
...
4. Done for first host adapter seam: expose shared comments-pane, accessibility-summary, alt-text request, and proofing plans through thin WPF/Avalonia command/state adapters. Modern comments, persistent alt text, full panes/dialogs, and proofing/accessibility execution remain open.
...
| 9 | Comments/accessibility UI realization | `FreeP.App.Presentation`, WPF/Avalonia host adapters/tests | First thin WPF/Avalonia host adapters expose shared comments-pane, accessibility-summary, alt-text request, and proofing plans. Next slices should turn those plans into real panes/dialogs and add persistent alt-text model/IO, modern resolved comments, proofing execution, and full accessibility checks without duplicating policy. | `PresentationReviewWorkflowPlannerTests`; `ReviewWorkflowAdapterTests`; `MainWindowHeadlessTests`; focused WPF/Avalonia adapter tests. |
