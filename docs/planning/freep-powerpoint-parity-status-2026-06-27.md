...
Branch/worktree context for this update: `codex/freep-comments-notes-corpus-20260701`, based on `origin/main` commit `2aa77a678`. This update reflects the 2026-07-01 FreeP parity implementation wave plus the comments/notes corpus-retention slice.
...
The first post-dedup FreeP parity implementation wave is integrated on `main` through `2aa77a678`, with this branch adding comments/notes corpus coverage. It closed the initial evidence and shared-planning gates that were blocking reliable WPF/Avalonia parity work:
...
- Expanded the Avalonia ribbon profile from the initial small surface to 67 shared command ids. The generated command inventory currently reports 94 total commands, 67 shared, 0 explicit Avalonia gaps, 19 known deferred commands, 6 platform-only commands, and 2 command-id aliases.
...
| Avalonia app | `freep/FreeP.App.Avalonia/**`, `freep/FreeP.App.Rendering.Avalonia/**`, `freep/FreeP.App.Avalonia.Tests/**`, `freep/FreeP.App.Rendering.Avalonia.Tests/**`. | Cross-platform shell and renderer exist. Avalonia consumes shared file workflow, ribbon definitions, slide pane planning/actions, rendering planners, shared presenter state, shared export planning, shared transition commands, and shared Design commands. No explicit Avalonia command gaps remain in the generated inventory; the larger Animations slice is intentionally classified as deferred. |
...
| --- | --- | --- | --- | --- |
| FreeP WPF-vs-Avalonia command matrix | The generated matrix now exists and is the source of truth for command-surface work. Current state: 94 total, 67 shared, 0 explicit Avalonia gaps, 19 known deferred, 6 platform-only, 2 command-id aliases. | Keep `tools/Generate-FreePCommandParityInventory.ps1` and generated docs current as command slices land. | WPF/Avalonia only classify adapter-only or platform-only commands. | `tools/Test-GeneratedDocs.ps1`; `FreePRibbonDefinitionProfileTests.cs`. |
| Ribbon depth and stubs | The explicit Avalonia command gaps are closed, and transition plus Design command IDs are exposed through WPF/Avalonia planner-backed routes. Animations, Review/View/Accessibility/Recording, and layout controls remain incomplete or deferred. | Shared ribbon definitions, command planners, layout/timing/animation models in `FreeP.App.Presentation`. | Host adapters render galleries, combo boxes, and dialogs. | Command inventory deltas; `RibbonEditorCompleteness5BTests.cs`; `RibbonTransitionsAnimationsTests.cs`; new stub-failure tests. |
...
| Slide pane and editing parity | Slide pane context actions, enabled-state, and drag-reorder action planning are now shared and consumed by WPF/Avalonia. Avalonia still has no table-cell editor equivalent. | `SlidePanePlanner`, editing/session/table-cell planners, gesture reducer. | WPF/Avalonia list/control realization and input wiring. | `SlidePanePolicySourceGuardTests.cs`; `SlidePaneTests.cs`; new Avalonia table-cell tests. |
...
2. Done for explicit gaps: expand Avalonia ribbon profile so Home clipboard/font/arrange/editing/slides, Insert shapes/tables/charts/links, Transitions, and Design are shared. Animations remain the main known-deferred ribbon slice.
...
| 6 | Table editing parity | `FreeP.App.Presentation`, `FreeP.App.Avalonia`, renderer adapters/tests | Avalonia gains WPF-equivalent table-cell editing through shared planners. | `SlidePane*` tests; new Avalonia table-cell tests. |
...
Current integration stance: FreeP has a substantial shared codebase, generated command inventory with zero explicit Avalonia command gaps, 21-deck package-retention contracts plus first semantic-edit corpus coverage for media, charts, SmartArt, comments, and notes, trustworthy render-harness failure reporting, initial shared chart/text/gesture planners, shared slide-pane action planning, shared presenter/slideshow state consumed by WPF and Avalonia, shared export/backstage planning with WPF/Avalonia PDF export routes, and shared transition plus Design command intents exposed through WPF/Avalonia. It is not PowerPoint-parity complete. The next work should generate missing PowerPoint refs on a COM-capable machine, then continue deeper shared planner work for Animations, text, charts, export/print depth, presenter recording/ink tools, table editing, modern comments/review workflows, and accessibility surfaces.
