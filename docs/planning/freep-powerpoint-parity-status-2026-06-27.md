...
Branch/worktree context for this update: `codex/freep-review-ui-adapter-slice-linear-20260701`, based on `origin/main` commit `12f5e7571`. This update reflects the 2026-07-01 FreeP parity implementation wave through the integrated table-cell editing planner, Avalonia table-cell overlay, presenter recording/ink intent, image export execution, and review workflow adapter slices.
...
The first post-dedup FreeP parity implementation wave is integrated on `origin/main` through `12f5e7571`, including `bc1d49b57` shared table-cell planning, `3451f903f` Avalonia table-cell overlay wiring, `322ada635` presenter recording/ink intents, `267cc43b2` shared image export execution, and `12f5e7571` review workflow host adapters. It closed the initial evidence and shared-planning gates that were blocking reliable WPF/Avalonia parity work:
...
- Added shared selected-slide PNG image export execution. `PresentationImageExportExecutor` owns slide-range normalization through the shared planner, deterministic output naming, atomic writes, empty-deck no-op behavior, and non-empty renderer output validation; WPF and Avalonia provide only thin render callback and caller-supplied folder path hooks.
...
| PDF/export/print/backstage | PowerPoint supports File > Export to PDF/video/handouts and print layouts for slides/handouts/notes. FreeP now has shared export descriptors, PDF dialog/picker plans, Backstage export plan, slide-range normalization, selected-slide PNG image export execution, deferred video plans, and print layout plans for full-page slides, notes pages, and handouts. WPF/Avalonia PDF export routes consume the shared PDF path, and WPF/Avalonia image export hooks are thin render-callback/folder-path adapters over `PresentationImageExportExecutor`. PDF output includes basic slide background and shape rectangle draw ops, but full compositor/vector fidelity, a full user-facing folder-picker/backstage image-export command flow, native print execution/preview, video encoding, and handout rendering remain open. | Shared export/backstage/print planners; `PresentationImageExportExecutor`; `PresentationPdfExporter` should continue moving toward compositor/draw-op-backed slide export. | Native print dialogs, owner windows, platform file/folder pickers, renderer callbacks. | `PresentationPdfExporterTests.cs`; `PresentationExportPlannerTests.cs`; PDF/render inspection. |
...
4. Done for shared image execution: selected-slide PNG export normalizes ranges, emits deterministic names, writes atomically, and handles empty decks without invoking host renderers.
5. Keep native print/file/folder dialogs, full Backstage command flow, and print execution platform-specific.
...
| 8 | Export/print/backstage depth | `FreeP.App.Presentation`, `FreeP.Core.IO`, WPF/Avalonia host adapters/tests | Existing shared export descriptors, selected-slide PNG execution, and print plans grow into a full user-facing folder-picker/backstage image-export command flow, native print execution/preview, handout rendering, video export, and stronger compositor-backed PDF fidelity. | `PresentationPdfExporterTests`; `PresentationExportPlannerTests`; PDF/render inspection. |
