...
| Shared workbook session and command orchestration | `src/FreeX.App.Services` | Keep core/shared | Keep session, open/save services, clipboard serializers, compact dialog planners, find/replace, sheet, formatting, Goal Seek, sort, data-validation mutation, export/print planning, and viewport orchestration portable. New macOS features should enter here when they are not UI-specific. |
...
| Printing, print preview, PDF, and XPS export | `PrintRenderer*`, `PrintPreviewDialog*`, `MainWindow.PrintExport.cs`, `PdfDocumentExporter.cs`, `PDFsharp-WPF`, ReachFramework/XPS, `System.Windows.Documents`; shared `WorkbookExportReadinessPlanner` and `WorkbookExportPrintPlanner` | Keep shared planning; defer / guard Windows-only renderer | Keep export readiness and PDF print planning in `FreeX.App.Services`. The shared contract plans PDF scopes/pages for macOS and rejects XPS there; the current WPF fixed-document/XPS renderer remains Windows-only until a native Avalonia/macOS PDF/print renderer lands. |
| Windows Share / macOS share fallback | `WorkbookShareReadinessPlanner.cs`, `WorkbookShareActionPlanner.cs`, `ShareWorkbookPlanner.cs`, `WindowsWorkbookShareService.cs`, `MainWindow.ReviewCommands.cs` with WinRT `DataTransferManager`, `StorageFile`, COM interop, and `WindowInteropHelper` | Keep planners shared; replace adapter / defer | Keep save-before-share planning reusable in `FreeX.App.Services`. `WorkbookShareActionPlanner` maps saved/unsaved local workbook state plus injected share-sheet/open-containing-folder capabilities to share-sheet, open-containing-folder, save-first, or deferred outcomes without platform API references. Avalonia File > Share Workbook now uses the planner, saves dirty/current workbooks when needed, routes unsaved or missing paths through Save As first, and falls back through Avalonia `TopLevel.Launcher` to Reveal in Finder on macOS or open-containing-folder elsewhere. Native share-sheet integration remains deferred. |
| WPF modal/modeless dialogs | Many `*Dialog.xaml`, `*Dialog.xaml.cs`, and code-built dialogs under `src/FreeX.App.Host`; compact Avalonia routes now cover Format Cells, Find/Replace/Go To, Goal Seek, and selected-range Data Validation | Replace with Avalonia/native macOS | Port dialog behavior by priority, not file-for-file. Reuse existing input parsers/planners and compact shared planners such as `GoalSeekRequestParser`, `DataValidationPresetPlanner`, and `DataValidationPreviewPlanner`; keep WPF dialog tests as Windows coverage. |
...
3. Leave native macOS PDF/print rendering, Windows Share, full WPF dialog parity, WPF UIA, Excel COM, and Windows packaging as separate guarded or deferred lanes until the preview shell needs them.
