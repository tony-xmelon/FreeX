...
| P0 | User prompts, compact dialogs, and accessibility IDs | WPF uses `Window`, `MessageBox`, `DialogButtonRowFactory`, focus helpers, and UI Automation coverage. | Use compact Avalonia dialogs and automation IDs for preview-critical flows. Full WPF dialog layout, access-key, focus-ring, and screen-reader parity remain evidence-driven follow-up work. | `src/FreeX.App.Host/WpfUserMessageService.cs`; `src/FreeX.App.Host/DialogMessageHelper.cs`; `src/FreeX.App.Host/DialogButtonRowFactory.cs`; `src/FreeX.App.Host/DialogFocus.cs`; `src/FreeX.App.Avalonia/MainWindow.cs`; `docs/planning/macos-accessibility-evidence.md` | `tests/FreeX.App.Services.Tests/AvaloniaShellSourceTests.cs`; WPF dialog/accessibility tests continue to guard Windows; human checklist evidence through `tools/Test-MacOsHumanValidationChecklist.ps1` |
| P1 | Share workbook and reveal in Finder | Windows has a WinRT share adapter using `DataTransferManager` and window interop. | Keep the shared planner and current fallback: saved workbooks can open their containing folder; unsaved, missing, invalid, cloud, or web-link paths route to Save As first with explicit readiness/status messaging. A native macOS share sheet needs a host adapter before it can move from fallback to preview promise. | `src/FreeX.App.Host/WindowsWorkbookShareService.cs`; `src/FreeX.App.Avalonia/WorkbookShareSheetService.cs`; `src/FreeX.App.Services/WorkbookShareReadinessPlanner.cs`; `src/FreeX.App.Services/WorkbookShareActionPlanner.cs`; `src/FreeX.App.Avalonia/MainWindow.cs` | `tests/FreeX.App.Services.Tests/WorkbookShareReadinessPlannerTests.cs`; `tests/FreeX.App.Services.Tests/WorkbookShareActionPlannerTests.cs`; `tests/FreeX.App.Services.Tests/AvaloniaShellSourceTests.cs`; macOS human evidence before claiming native share-sheet support |
| P1 | Drag and drop open | WPF handles `DragEventArgs`, `DataFormats.FileDrop`, and `WorkbookDropPlanner`. | Avalonia file-drop wiring is integrated for existing local workbook files and routes through the shared open pipeline with busy/dirty guards. File activation and open panel remain the P0 paths; Finder drag/drop behavior still needs human macOS validation before preview promotion. | `src/FreeX.App.Host/MainWindow.FileDrop.cs`; `src/FreeX.App.Host/WorkbookDropPlanner.cs`; `src/FreeX.App.Avalonia/MainWindow.cs`; `src/FreeX.App.Services/WorkbookFileAdapterCatalog.cs`; `docs/release/macos-public-preview-checklist.md` | `tests/FreeX.App.Host.Tests/WorkbookDropPlannerTests.cs`; `tests/FreeX.App.Services.Tests/AvaloniaShellSourceTests.cs`; `tools/Test-MacOsHumanValidationChecklist.ps1` |
| P1 | External/internal links, help, about, and legal notices | WPF launches browser URLs, follows workbook hyperlinks, and shows host dialogs. | Use Avalonia `TopLevel.Launcher` for app help/feedback/legal links and compact dialogs for about/legal text. Workbook `Open Hyperlink` now routes through `WorkbookSession`: `PlaceInThisDocument` links navigate internally, while external workbook hyperlinks return an unsupported message until a safe platform-launch adapter exists. Treat localization and richer help chrome as P1 follow-up. | `src/FreeX.App.Host/ExternalUrlLauncher.cs`; `src/FreeX.App.Avalonia/MainWindow.cs`; `src/FreeX.App.Services/HyperlinkNavigationPlanner.cs`; `src/FreeX.App.Services/WorkbookSession.cs`; `src/FreeX.App.UI/IUserMessageService.cs` | `tests/FreeX.App.Host.Tests/ExternalUrlLauncherTests.cs`; `tests/FreeX.App.Services.Tests/AvaloniaShellSourceTests.cs`; `tests/FreeX.App.Services.Tests/HyperlinkNavigationPlannerTests.cs`; `tests/FreeX.App.Services.Tests/WorkbookSessionTests.cs` |
