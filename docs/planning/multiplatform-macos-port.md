...
The preview shell can open startup-argument workbooks, use in-app and native File menu Open commands for local workbook files, save through the native File menu Save and Save As commands, accept dropped local workbook files in the app window, size the workbook viewport from the visible app surface, move the active cell with keyboard navigation, and pan the workbook viewport from selection movement or mouse-wheel gestures. The app bundle now advertises Finder document types for `.fxl` plus common spreadsheet formats; macOS open-document events for an already-running app are still deferred.

This is a preview artifact, not a release channel. Public distribution still needs a macOS icon, Developer ID signing, notarization, checksum instructions for testers, and a broader macOS UI/accessibility test plan.
