...
The current Paste Special preview also includes comments/notes, data-validation metadata, and content-kind variants for All Except Borders, All Merging Conditional Formats, Formulas and Number Formats, Values and Number Formats, and Values and Source Formatting through the shared `WorkbookSession` and native menu. `All Using Source Theme` remains deferred until it has distinct core theme semantics. Remaining full Paste Special parity primarily means external text/Unicode Text choices, picture/linked-picture paste, multi-range clipboard behavior, and fuller dialog/access-key parity.
