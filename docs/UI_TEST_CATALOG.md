...
| UI-CAT-QAT-001D | UI-CAT-SHELL-001 | Customize QAT command-list search/filter, order, reset, import/export, and above/below-ribbon location. | In Progress - Options dialog source/XAML/runtime coverage proves persisted Quick Access Toolbar command customization, command-list search/filter, reset, add/remove/reorder, FreeX-owned `.freex-qat.json` import/export, and below-ribbon location; planner/source coverage now proves eligible direct ribbon commands expose add/remove QAT context-menu actions. |
