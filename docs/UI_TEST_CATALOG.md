...
| XAML click-wired controls | 631 | `Click="..."` occurrences in `MainWindow.xaml` on latest synced `origin/main`. |
...
| UI-CAT-DRAW-001 | Drawing commands | Rectangle, ellipse, line, bring/send, size/rotation, fill/outline, alt text, crop/reset, gradients/effects, Selection Pane. | Mouse, keytips, dialogs, list keyboard traversal, UIA patterns. | Picture, shape, text box, chart/drawing object, multiple objects where supported. | Live host keytip coverage now proves Insert Shapes routes Rectangle, Ellipse, and Line into drawing objects at the active cell. Source coverage also routes picture, shape, alt-text, z-order, and Selection Pane context actions to existing host commands. Mouse handles, visual render, z-order mutation, and persistence remain. | In Progress |
...
| UI-CAT-INSERT-002A | UI-CAT-INSERT-002 | Insert/change chart | Chart family menus, supported/deferred/excluded families, render and selected chart target. Live host coverage now proves `Alt,N,CC` inserts a Column chart from the selected range and collapsed `Alt,N,CH,MP` reaches Map Chart without mutating the workbook, showing the owned deferred-family message instead. Change Chart Type, selected chart handles, visual render, and persistence remain. | In Progress |
...
| UI-CAT-RIBBON-002A | UI-CAT-SHELL-002 | Collapsed group overflow | Narrow-window collapsed Home Editing, Insert Charts, and View Window groups open menus with live checked/enabled state and input gesture text. Live host coverage now includes Insert Charts Map Chart deferred routing from the collapsed overflow. | In Progress |
...
| UI-CMD-INSERT-005 | UI-CAT-INSERT-002 | Charts | Embedded/chart sheet, supported families, collapsed Insert > Charts keytip overflow routing (`Alt,N,CH`) with Recommended Charts, Column Chart, and Map Chart child keytips. Live host coverage now proves `Alt,N,CC` inserts a Column chart model from the selected range and collapsed `Alt,N,CH,MP` reaches the deferred Map command without mutating charts. Insert Chart recommended-gallery default focus, Change Chart Type subtype-gallery default focus, Chart Titles title-box default focus, Chart Styles gallery default focus, Move Chart target-choice default focus, Select Data range-box default focus, Chart Area fill-box default focus/select-all, Data Labels show-choice default focus, Series selector default focus, Trendline show-choice default focus, Error Bars show-choice default focus, Axis minimum-box default focus/select-all, axes, series, trendlines, error bars remain. | In Progress |
| UI-CMD-INSERT-006 | UI-CAT-INSERT-002 | Deferred/excluded chart families | Surface/treemap/sunburst/histogram/waterfall/funnel/map/3D disabled or blocked with clear rationale. Map Chart now stays enabled and visible in Insert Charts, exposes the deferred tooltip copy, and routes through the owned deferred-family info message without workbook mutation. Other deferred/excluded chart-family UX remains. | In Progress |
...
| UI-CAT-INSERT-002D-H | UI-CAT-INSERT-002 | Supported chart families; stock/radar; deferred advanced chart families; Recommended Charts excluded; sparklines. Column Chart now has live keytip insertion evidence, Map Chart remains visible in Insert Charts and routes to an owned deferred-family explanation, and source/planner coverage guards the deferred tooltip and enabled command path. Other supported families, Recommended Charts exclusion, sparklines, visual render, and save/load breadth remain. | In Progress |
...
| UI-CAT-DRAW-001D-Q | UI-CAT-DRAW-001 | Rectangle; Ellipse; Line; Bring Forward; Send Backward; Size/Rotation; Fill Color; Outline Color; Alt Text; Crop; Gradients/Effects; Selection Pane; Freehand Ink excluded; interactive drag handles deferred. Insert > Shapes now proves Rectangle, Ellipse, and Line insertion through keyboard keytips at the selected cell. Object context-menu source coverage routes picture/shape sizing, rotate, crop/reset crop, fill/outline, bring/send, alt text, and Selection Pane actions to existing commands. Draw-tab formatting, z-order mutation, persistence, and visual rendering proof remain. | In Progress |
