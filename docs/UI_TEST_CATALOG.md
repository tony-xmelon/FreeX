...
|---|---:|---|
| Command surface in-scope rows | 197 | From `COMMAND_INVENTORY.json`: Implemented + Partial command-surface rows. |
| Menu/toolbar in-scope rows | 198 | Includes the current Draw tab menu/toolbar delta. |
...
| XAML click-wired controls | 625 | `Click="..."` occurrences in `MainWindow.xaml` on latest synced `origin/main`. |
| Explicit UIA automation ids | 55 | `AutomationProperties.AutomationId="..."` declarations in `MainWindow.xaml`. |
| Ribbon keytip metadata declarations | 635 | `RibbonTooltip.KeyTip="..."` declarations in `MainWindow.xaml`. |
...
| UI-CMD-INSERT-006 | UI-CAT-INSERT-002 | Deferred/excluded chart families | Surface/treemap/sunburst/histogram/waterfall/funnel/3D families remain supported or blocked according to current model coverage. Map Chart is recognized for preservation but hidden from Insert Charts until authoring/rendering exists, so no disabled/deferred ribbon command is exposed. Other deferred/excluded chart-family UX remains. | In Progress |
...
| UI-CAT-QAT-001D | UI-CAT-SHELL-001 | Customize QAT command-list, order, reset, and above/below-ribbon location. | In Progress - Options dialog source/XAML coverage proves persisted Quick Access Toolbar command customization, reset, add/remove/reorder, and below-ribbon location. |
...
| UI-CAT-INSERT-002D-H | UI-CAT-INSERT-002 | Supported chart families; stock/radar; deferred advanced chart families; Recommended Charts excluded; sparklines. Column Chart now has live keytip insertion evidence, Map Chart is hidden from Insert Charts until authoring/rendering exists, and source/catalog coverage guards that it is not exposed as a deferred command. Other supported families, Recommended Charts exclusion, sparklines, visual render, and save/load breadth remain. | In Progress |
...
| UI-CAT-VIEW-002A-H | UI-CAT-VIEW-002 | Freeze Panes; Split; Zoom; Zoom to Selection; 100% Zoom; Arrange All; New Window and Switch Windows now use live registry-backed handlers. Hide, Unhide, View Side by Side, Synchronous Scrolling, and Reset Window Position are removed from the ribbon until workbook-window visibility and paired-window subsystems exist. Freeze Panes menu keytips now execute top-row, first-column, selection-based, and unfreeze commands against model state; Split keytips toggle selected-cell split state. | In Progress |
