...
- Microsoft cloud services, online templates, co-authoring, identity, SharePoint-only workflows, and web-backed linked data. This includes Microsoft 365's sheet-tab **Link to this Sheet** command: it creates a permissioned web-sharing link, not an offline workbook hyperlink.
...
| `tools/ux-parity-runs/20260823-125437/ux-scenario-batch.json` | Repaired smoke run completed 2/2 paired captures with zero blocked records. The capture tool now rejects focused child windows as popups and identifies Excel sheet tabs by UIA `TabItem`, so the Excel image is the real sheet-tab menu. Both Format Cells dialogs use the same validated `score, 1, 2, 3` fixture. The live Excel menu's remaining **Link to this Sheet** row is explicitly excluded as a Microsoft 365 web-sharing feature. |
