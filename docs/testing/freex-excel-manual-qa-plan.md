...
| 16 | `20260831-143234` Find Next keyboard behavior | Passed with visual/workflow disparity | Excel-first then FreeX: `Ctrl+F`, enter `smoke`, and press Enter finds `UX Overview!B5` in both applications. FreeX immediately expands a result grid after this single Find Next action, while Excel retains its compact Find dialog; see `UX-QA-ITER16-001`. |
| 17 | `20260831-144227` Charts title/default-style revalidation | Partial pass | The rebuilt host preserves the fully visible chart and now matches Excel's bold dark title more closely. FreeX still uses a different default series palette and automatic value-axis maximum/major units (`5720`/`1430` versus Excel's `7000`/`1000`), so visual parity remains open; see `UX-QA-ITER12-001`. |
| 18 | `20260831-144227` ribbon Alt keytips revalidation | Failed revalidation | FreeX's visible Home-command labels are no longer crowded, but the bare-Alt map omits the visible keys for most ribbon tabs that Excel exposes (`Insert`, `Draw`, `Page Layout`, `Formulas`, `Data`, `Review`, `View`, and others). Keyboard access to the complete ribbon surface remains open; see `UX-QA-ITER5-001`. |
