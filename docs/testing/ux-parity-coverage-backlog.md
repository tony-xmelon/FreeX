...
| Grid selection, edit, fill | Click, drag, Shift/Ctrl selection, row/column headers, edit mode, formula edit, fill handle, autofill options, resize, wheel scroll. The foreground batch now exposes a COM-independent `-Suite grid` that records FreeX row/column resize and wheel-scroll workflow evidence through the same manifest/contact-sheet path as paired suites; drag selection, autofill handle drag, and double-click autofit remain individual scenarios pending screenshot-capture hardening before default batch promotion. | Run `-Suite grid` on a foreground-capable desktop to collect FreeX resize/scroll evidence, then add matching Excel reference captures for final paired visual closeout plus drag selection, autofill, edit commit/cancel, and formula/reference variants. |
...
| Formula bar and name box | Name navigation, defined names, formula entry, `fx`, expand/collapse, reference highlighting, error/invalid entry. Dedicated reference evidence now exists in `tools/ux-parity-runs/20260702-formula-bar-name-box-reference-v3/ux-scenario-batch.json`: both apps select `B4` on the same seeded formula worksheet and FreeX validates Name Box `B4` plus Formula Bar `=B2-B3` through UIA before capture. | Continue with paired Insert Function, formula bar expand/collapse, edit commit/cancel, reference highlighting, invalid formula recovery, and defined-name navigation. |
| Dialogs | Format Cells, Sort, Data Validation, Page Setup, Insert Function, Find/Replace, Options-style dialogs, invalid input, OK/Cancel/Escape. The current strict core run `tools/ux-parity-runs/20260702-core-after-context-and-number-pane/ux-scenario-batch.json` captures improved Excel/FreeX Format Cells parity after tab-order, size, General-description, selected-cell preview, Number-list density, highlight, compact right pane/sample frame, and compact OK/Cancel fixes. | Continue Format Cells visual tuning for exact foreground/DPI geometry, then convert the pair into a dialog batch with access-key, default button, focus order, and UIA pattern checks. |
| Status bar | Ready/edit modes, statistics, zoom slider/buttons, view shortcuts, selection counts, accessibility names. Paired status/footer stats evidence now exists in `tools/ux-parity-runs/20260702-status-footer-reference-v2/ux-scenario-batch.json` with validated Excel Average/Count/Sum and FreeX Average/Count/Numerical Count/Sum/Min/Max. | Continue with visual parity triage for extra FreeX readouts/capture geometry, then pair zoom min/max, Ctrl+wheel zoom, and footer view buttons. |
| Sheet tabs | Add, rename, delete, duplicate, move, color, hide/unhide, group/ungroup, context menu. Strict paired sheet-tab context and real sheet-list overflow Activate captures now exist in `tools/ux-parity-runs/20260702-core-after-context-and-number-pane/ux-scenario-batch.json`; `20260702-core-activate-list-40-pass` is the pre-context-menu/Number-pane baseline, `20260702-format-cells-density-pass` is the pre-Activate-density baseline, `20260702-core-after-dialog-fidelity` is the pre-density-pass baseline, `20260702-021746` is the pre-fix baseline, `20260702-012034` is superseded because visual review found a worksheet-menu false positive, and `20260702-015812` is superseded because visual review found the Excel workbook/window Activate fallback. | Continue with sheet-tab context-menu visual scale/placement parity, Tab Color submenu-palette behavior, remaining Activate selected-row highlight/framing polish, drag reorder, grouped sheets, keyboard/context-menu routes, and broader visual review of the paired screenshots. |
...
| Sorting and filtering | Simple/custom sort, table/filter flyouts, search, checklists, date/number/text filters, clear/reapply, visible rows. The foreground batch now exposes `-Suite filtering`, pairing Excel `excel-autofilter` with FreeX `freex-autofilter` against the same A1:D6 seeded range; the FreeX half validates the Text Filters checklist surface after `Ctrl+Shift+L` and `Alt+Down`. | Run the filtering suite on a foreground-capable desktop, then extend it with filter selection/clear/reapply, custom sort, search, date/text criteria, and before/after visible-row state. |
