...
subset including scalar comparisons, truthiness predicates, zero-argument `TRUE()`/`FALSE()` functions, simple `IS*` predicates including `ISREF`/`ISFORMULA`, simple `IF` wrappers, and simple `XOR` logical wrappers, low-contrast text boxes against
