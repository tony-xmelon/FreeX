...
subset including scalar comparisons, simple `+`/`-`/`*`/`/` arithmetic operands, bounded `SUM`/`AVERAGE`/`MIN`/`MAX`/`COUNT`/`COUNTA` aggregate operands, truthiness predicates, zero-argument `TRUE()`/`FALSE()` functions, simple `IS*` predicates including `ISREF`/`ISFORMULA`, simple `IF` wrappers, and simple `XOR` logical wrappers, low-contrast text boxes against
