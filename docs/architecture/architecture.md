...
- **Core.IO**: `NativeJsonAdapter` (.fxl — compact JSON, SHA-256 password hashing via `ProtectionPasswordHelper`), `XlsxFileAdapter` (ClosedXML 0.105.0 — stream-load, structured load warnings via `XlsxLoadResult`), `CsvFileAdapter`, delimited-text adapters, `SpreadsheetXmlFileAdapter` for Excel XML Spreadsheet 2003 `.xml`, `XsltWorkbookTransform` for safe XSLT-to-SpreadsheetML imports, `XmlNativeBagSerializer` for `NativeXmlPreserveBag` round-trip serialisation, and `OdsFileAdapter` (`.ods` — read/write, registered in `WorkbookFileAdapterCatalog`); the parked research in `docs/formats/ods-open-support-research.md` is superseded by this in-house adapter.
