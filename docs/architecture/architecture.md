...
subset including scalar comparisons, simple arithmetic operands including unary negate/percent, finite `^`, and `+`/`-`/`*`/`/`, common scalar numeric functions (`ABS`, `INT`, `ROUND`, and `MOD`), text functions (`LEN`, `UPPER`, `LOWER`, `TRIM`, `LEFT`, `RIGHT`, `MID`, `FIND`, `SEARCH`, `EXACT`, and `VALUE`), date functions (`DATE`, `YEAR`, `MONTH`, `DAY`, `TODAY`, and `NOW`), and row/column functions (`ROW` and `COLUMN`), bounded `SUM`/`AVERAGE`/`MIN`/`MAX`/`COUNT`/`COUNTA`/`COUNTBLANK` aggregate operands over literals, shifted references, scalar direct operands, nested bounded aggregate arguments, and guarded finite ranges, truthiness predicates, zero-argument `TRUE()`/`FALSE()`/`NA()` functions, simple `IS*` predicates including `ISREF`/`ISFORMULA`, simple `IF` wrappers, and simple `XOR` logical wrappers, low-contrast text boxes against
