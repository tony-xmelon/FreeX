...
subset including scalar comparisons, simple arithmetic operands including unary negate/percent, finite `^`, and `+`/`-`/`*`/`/`, common scalar numeric functions (`ABS`, `INT`, `EVEN`, `ODD`, `ROUND`, `ROUNDUP`, `ROUNDDOWN`, `TRUNC`, `FACT`, `FACTDOUBLE`, `MROUND`, `CEILING`, `CEILING.MATH`, `CEILING.PRECISE`, `ISO.CEILING`, `FLOOR`, `FLOOR.MATH`, `FLOOR.PRECISE`, `MOD`, `QUOTIENT`, `COMBIN`, `COMBINA`, `PERMUT`, `PERMUTATIONA`, `GCD`, `LCM`, `SQRT`, `SQRTPI`, `SIGN`, `POWER`, `EXP`, `LN`, `LOG`, `LOG10`, `PI`, `DEGREES`, `RADIANS`, `SIN`, `CSC`, `CSCH`, `SINH`, `ASINH`, `ACOSH`, `COSH`, `SECH`, `TANH`, `ATANH`, `ACOTH`, `COTH`, `ASIN`, `ACOS`, `ACOT`, `COS`, `SEC`, `COT`, `TAN`, `ATAN`, and `ATAN2`), text functions (`LEN`, `UPPER`, `LOWER`, `TRIM`, `LEFT`, `RIGHT`, `MID`, `FIND`, `SEARCH`, `EXACT`, and `VALUE`), date functions (`DATE`, `YEAR`, `MONTH`, `DAY`, `TODAY`, and `NOW`), and row/column functions (`ROW` and `COLUMN`), bounded `SUM`/`SUMSQ`/`AVEDEV`/`DEVSQ`/`PRODUCT`/`AVERAGE`/`MEDIAN`/`MIN`/`MAX`/`COUNT`/`COUNTA`/`COUNTBLANK`/`STDEV`/`STDEV.S`/`STDEVP`/`STDEV.P`/`VAR`/`VAR.S`/`VARP`/`VAR.P`/`GEOMEAN`/`HARMEAN` aggregate operands over literals, shifted references, scalar direct operands, nested bounded aggregate arguments, and guarded finite ranges, truthiness predicates, zero-argument `TRUE()`/`FALSE()`/`NA()` functions, simple `IS*` predicates including `ISREF`/`ISFORMULA`, simple `IF` wrappers, and simple `XOR` logical wrappers, low-contrast text boxes against
