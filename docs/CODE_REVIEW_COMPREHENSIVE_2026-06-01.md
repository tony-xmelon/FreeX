...
- **O1 — `FormattedText` per-cell allocation (was P1 perf): substantially addressed.** The render
  ...
  remove per-probe shrink allocation). Recommend a `PERF_BASELINE.md` measurement to close it
...
These remain as recorded in `OUTSTANDING_BUILD.md`; none are correctness/security/data-loss.
...
| P2 | O2 — pool transient `ScalarValue[,]`/argument buffers (needs `PERF_BASELINE.md`) | Perf | Deferred, needs baseline |
