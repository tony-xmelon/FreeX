...
| **Theme-color effects are partial** | Full OOXML theme-effect chains (reflection, bevel, 3D, etc.) are not interpreted; base theme colors plus bounded non-chart theme outer-shadow, inner-shadow, glow, and soft-edge defaults are applied. |
| **Advanced multi-window workbook view** | New Window, Switch Windows, Hide/Unhide, View Side by Side, Synchronous Scrolling, and Reset Window Position are registry-backed. Arrange All stores the workbook arrangement choice; automatic tiling for every Arrange All layout remains partial. |
