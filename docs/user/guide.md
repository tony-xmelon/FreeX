...
- Building first/last-name email aliases with learned constant domains and `.`, `_`, or `-` separators, including first-initial/last-name, last-name/first-initial, first/last-initial forms, three-part full-name middle-initial aliases such as `ada.b.lovelace@contoso.com` or `ablovelace@contoso.com`, and adjacent first/middle/last column sources that either use only the first/last names or include the middle initial.
