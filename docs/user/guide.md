...
Error Checking lists deterministic workbook issues such as cached formula errors, inconsistent formulas, text-number/date warnings including apostrophe-prefixed, percent, and accounting-parentheses number text, blank references, omitted aggregate cells, unlocked formula cells, and invalid data-validation entries. Use Ignore Error for a selected issue or File -> Options -> Formulas to change the supported rule set.
