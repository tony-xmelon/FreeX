...
- Extracting web and email domain pieces, including hosts, root-domain stems from variable-depth subdomains, ampersand- or semicolon-separated query names, first values, and same-name values, decoded URL fragment identifiers, and final URL path segments such as `road-bike?ref=nav` to `road-bike`.
