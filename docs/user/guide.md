...
- Building first/last-name email aliases with learned constant domains and `.`, `_`, or `-` separators, including first-initial/last-name, last-name/first-initial, first/last-initial forms, three-part full-name middle-initial aliases such as `ada.b.lovelace@contoso.com` or `ablovelace@contoso.com`, and adjacent first/middle/last column sources that use the first and last name while ignoring the middle column.
