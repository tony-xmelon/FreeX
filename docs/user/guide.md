...
- Extracting web and email domain pieces, including hosts, root-domain stems from variable-depth subdomains, ampersand- or semicolon-separated first/last query names, first/last values, same-name first values, and same-name last repeated values, decoded URL fragment identifiers, and final URL path segments such as `road-bike?ref=nav` to `road-bike`.
