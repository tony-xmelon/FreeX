...
- Building first/last-name email aliases with learned constant domains and `.`, `_`, or `-` separators, including first-initial/last-name, last-name/first-initial, first/last-initial forms, and adjacent first/middle/last column sources that use the first and last name while ignoring the middle column.
