...
- Extracting web and email domain pieces, including hosts, root-domain stems from variable-depth subdomains, ampersand- or semicolon-separated first/last query names, first/last values, and same-name values, decoded URL fragment identifiers, and final URL path segments such as `road-bike?ref=nav` to `road-bike`.
