...
- Extracting web and email domain pieces, including hosts, root-domain stems from variable-depth subdomains and curated multi-label public suffixes, ampersand- or semicolon-separated first/last query names, first/last values, same-name first values, and same-name last repeated values, decoded URL fragment identifiers, final URL path segments such as `road-bike?ref=nav` to `road-bike`, decoded parent URL path segments such as `/catalog/bikes/road-bike.html` to `bikes`, and first URL path segments such as `/catalog/bikes/road-bike.html` to `catalog`.
