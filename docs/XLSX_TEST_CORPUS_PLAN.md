...
| Formatting | 15 | Fonts, fills, borders, alignment, wrapping, number formats, style-only empty cells | Style IDs, rendered display text, cell style properties, plus Excel-visible smoke counters for styled cells, bordered cells, and non-General number-format cells |
...
7. Verify feature tags that have deterministic model or Excel-visible signals, such as named ranges, hyperlinks, merged cells, chart counts, formatting/number-format/border counters, structure counters, page setup/protected-range counters, and cell-type content.
