...
The probe then physically clicks the collapsed Advanced Animation group and its rendered flyout item on the real Animations ribbon to open the pane, captures the flyout, visible docked pane, and seeded row, clicks the row's order/name area through X11 pointer input, and uses the same physical route to close and reopen the pane. The manifest row `animation-pane-physical-workflow` requires all five observable transitions: pane open, seeded row visible, row selection changed, pane closed, and pane reopened.
