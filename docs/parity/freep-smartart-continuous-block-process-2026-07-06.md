# FreeP SmartArt Continuous Block Process - 2026-07-06

## Scope

This slice admits PowerPoint `continuousBlockProcess` diagrams into the bounded
FreeP SmartArt live-layout allow-list.

The reader already classifies the layout as `Process`; this change keeps that
classification and enables the existing shared process planner for the specific
common continuous-block variant. Unsupported process-family variants remain
disabled for live planning and continue to use cached drawing fallback.

## Shared-First Behavior

- `PptxPackageReader` marks `continuousBlockProcess` as live-layout supported.
...
- `SmartArtLayoutTests` proves `continuousBlockProcess` produces live boxes and
...
- `SmartArtTests` proves the PPTX reader enables live layout for
  `continuousBlockProcess` while disabling an unsupported process variant.
