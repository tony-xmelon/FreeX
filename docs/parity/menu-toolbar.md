...
| Options | Partial | General, Formulas, Proofing custom dictionary, View, and Save subsets including calculation/error-checking, formula bar preferences, and add/remove/clear custom dictionary management; sidebar categories, editable fields, option toggles, and OK/Cancel expose keyboard access keys; FreeX visual evidence captures default General category-list focus, Formulas/Quick Access Toolbar/View category navigation, and close-to-Backstage Options focus return in `screenshots/options-account-tour/options_account_tour_manifest.json` |
...
| Account | Partial | Local Account backstage action shows FreeX user name, Windows account, device, app version, options file, current workbook save/path status, Windows Share readiness, local PDF/XPS export readiness, and explicit Microsoft 365 sign-in/cloud-link/coauthoring exclusion; no Microsoft account integration; FreeX visual evidence captures the Backstage Account entry, real owned local-account message, and Account focus return in `screenshots/options-account-tour/options_account_tour_manifest.json` |
