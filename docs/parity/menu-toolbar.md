...
| Item | Status | Notes |
|---|---|---|
| Spell Check | Partial | Expanded common office and spreadsheet/business-report misspelling known-corrections catalog scans across text cells, notes, threaded comment roots, and threaded comment replies with casing-preserving replace, replace-all including repeated-word run collapse, case-insensitive Ignore All, persisted Add to Dictionary custom dictionary suppression across sessions/workbooks via FreeX options, and internet/email/file-address span skipping including quoted/bracketed file paths with spaces, plus report and finance typo corrections such as `availible`, `forcasting`, `statment`, `balence`, `ammount`, `comparision`, `transfered`, `liabilty`, `recievable`, `payrole`, `quaterly`, `depreciaton`, and `reconcilliation`; no full dictionary/proofing engine |
