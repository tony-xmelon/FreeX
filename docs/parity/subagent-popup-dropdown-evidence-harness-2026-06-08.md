...
- Added a FreeX Home Borders dropdown capture path: `FREEX_HOME_BORDERS_DROPDOWN_TOUR=1` opens the production Home Borders menu, captures `screenshots/home-borders-dropdown-tour/freex_dropdown_home_borders_opened.png`, and writes `home_borders_dropdown_tour_manifest.json` with pair key `interactive:home-borders:opened`. The paired Excel `Alt,H,B` route is source-wired as `FREEX_EXCEL_HOME_BORDERS_DROPDOWN_TOUR=1`, but live crop validation is not yet stable enough to check in an Excel artifact.
...
- `interactive:home-borders:opened`: FreeX exposes a long top-level Borders menu with all/outline/inside/no-border presets, individual edge presets, thick/double/top-bottom variants, draw/erase modes, Line Color and Line Style submenus, and `More Borders...`. A diagnostic Excel full-screen probe showed Excel's `Alt,H,B` menu opens as an Office `Net UI Tool Window` with visible keytip badges, but the foreground-guarded rectangle crop is still timing/composition-sensitive and is left as a follow-up before paired Excel artifact check-in.
