...
| 15 | Formula auditing, error checking, evaluate formula, and Watch Window | Formulas / Formula Auditing | Parity evidence | Parity evidence | Needs live pass | Needs live pass | Needs guard | Needs guard | Needs live pass | N/A | `FormulaAuditingServiceTests.*`, `WatchWindowServiceTests`, `ErrorCheckingDialogTests`, formula diagnostics tour evidence | Validate precedent/dependent arrows and watch add/delete/refresh across ranges, tables, hidden rows, filtered rows, protected sheets, and cross-sheet references. |
...
| 10 | Formula auditing and Watch Window over cross-sheet ranges, hidden/filtered rows, and protected sheets | Diagnostic commands need target breadth because mistakes can hide dependency arrows or stale watched-cell state without changing worksheet values. |
