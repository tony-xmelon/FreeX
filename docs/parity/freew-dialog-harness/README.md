...
- Comparison: 142 genuine visual mismatches, 64 visual passes, 70 Avalonia extensions, and 0 state-not-applicable rows. The current source also reports 15 semantic mismatches, with 0 pending WPF-factory and 0 invalid-capture-content rows. These totals are checked against the canonical comparison JSON by `tools/Test-FreeWDialogVisualEvidence.ps1`; they are not maintained independently.
