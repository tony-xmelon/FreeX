...
- 587 command IDs total.
- 587 shared across WPF and Avalonia.
...
bus, with matching WPF and Avalonia ribbon registrations. Solid, gradient, and pattern fills plus
solid and gradient outlines preserve their authored structure while only the alpha changes; the
model and existing PPTX reader/writer remain authoritative for package persistence. Focused
Presentation, WPF, Avalonia, and generated-command-inventory coverage verifies the routes and undo
behavior. This is a functional/package authoring slice and makes no new raster-fidelity claim.
