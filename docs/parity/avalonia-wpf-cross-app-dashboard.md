...
|---|---|---|---|
| FreeX | Functional matrix, classifier, dialog inventory, dialog visual evidence, command surface | 546 functional commands; 488 command inventory parity; 0 Avalonia-missing; 0 real classified binding gaps; 48 catalog-backed pseudo-gallery rows (34 conditional-format, 10 font/border, 4 accounting-symbol); 57/57 dialog routes captured on WPF and Avalonia; 94 paired screenshot surface ids, 0 Avalonia-only ids, 0 WPF-only ids; 0 scale-aware dimension mismatches; 0 unresolved high-delta visual review candidates at triage score >= 0.4 (highest 0.103523); 28 raw PNG pixel dimension mismatches, of which 28 normalize by capture DPI. These are coverage/triage metrics, not a visual-parity claim. | Command/dialog route coverage is complete for the generated inputs: all 48 pseudo-gallery rows are catalog-backed in classifier evidence (34 conditional-format rows over 38 runtime catalog items, 10 font/border rows, and 4 accounting-symbol rows), and dialog screenshot evidence has 94 paired WPF/Avalonia manifest surface ids with 0 Avalonia-only ids. This is coverage and size-comparability evidence, not visual parity. The paired screenshots retain 0 unresolved high-delta visual review candidates at triage score >= 0.4 (highest 0.103523); keep those candidates separate from 0 scale-aware dimension mismatch rows and 28 raw PNG pixel dimension mismatches. |
