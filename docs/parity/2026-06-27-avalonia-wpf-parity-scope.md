...
3. Remaining behavior/evidence work should focus on Word-like results that are weakly proven, especially citation formatting/source-manager depth beyond the modeled document-local fields, TOA output/evidence polish beyond the now-shared Mark Citation dialog, finer-grained protection history classification, rich rendering, and visual evidence. Proofing language apply now has shared caret/range behavior and dialog planning, cross-reference refresh is shared, type-aware source entry is shared, Mark Citation category/short-citation entry is shared, and generic protection history mutation is guarded in both hosts, so new work should target unproven behavior or evidence rather than duplicating those contracts.
