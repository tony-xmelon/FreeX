...
> Caveat: coverage is measured at the *command-binding* layer of each shell. `WPF-MISSING` rows are dominated by controls the WPF host drives through a non-Click path (combo-boxes like Font / Number Format / Scale*, the Help-tab buttons, and conditional-format icon-set gallery items) rather than a genuine WPF feature gap. The gate only fires on `AVALONIA-MISSING`.
