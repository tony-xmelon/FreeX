...
- Generated retained ink metadata records the source `Slide.Id` and optional custom-show name, so both hosts preserve where the ink was authored without needing a real device capture backend.
- WPF `SlideShowWindow` and Avalonia `SlideShowWindow` pass the shared playback route name into the same persistence planner during teardown.
