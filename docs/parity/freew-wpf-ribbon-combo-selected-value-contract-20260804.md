...
a compatibility fallback. Font family, font size, and line spacing also expose their effective caret or
paragraph value through `IRibbonStatefulCommand`, so loaded documents and re-renders update the controls.
...
- `RibbonComboCommandContractTests`: 2/2 passed against the actual shared selected-value context. The tests
  ...
  verify initial/post-apply font-family, font-size, and line-spacing ribbon values.
