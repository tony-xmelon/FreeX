...
The probe does not measure automatic combo-close focus before it physically reselects `A1` through the worksheet grid and sends `Right` followed by `Ctrl+C`; `B1` must be copied as `Unchanged`, proving subsequent worksheet keyboard routing after the explicit reselect. It then inspects the saved XLSX package and requires the target cell style to reference a font whose `<name>` value is `Arial`. The runner's `resume-provenance.json` records the exact worker source commit, published payload fingerprint, and owned Docker image used for the final evidence. Automatic combo-close focus remains unresolved; the explicit worksheet reselect is not evidence of automatic focus parity.
