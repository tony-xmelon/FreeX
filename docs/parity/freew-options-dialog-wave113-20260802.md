...
The existing dialog harness was run against the filtered 14-scenario Options inventory. Only `options.*` rows were merged into the tracked comparison artifacts. The 287 unrelated canonical rows were serialized and hashed before and after merge:
...
- Dialog harness: options route captured 8 WPF and 6 Avalonia scenarios; route merge wrote the canonical JSON, Markdown, HTML, and freshness artifacts. The harness exits nonzero when genuine visual mismatches remain, as expected for this cross-renderer comparison.
