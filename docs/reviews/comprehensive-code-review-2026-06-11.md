...
### Resolution update - 2026-06-12 second pass

The remaining backlog above was cleared the same day through twelve more `fix/*-20260612` branches in three integration waves:

- **Stability/correctness**: `_isOpeningFile` guard set before awaits with try/finally; clipboard image paste catch narrowed to decode with diagnostics; picture/background/header-footer image loads made async; screenshot-tour tasks wrapped with logging and CI shutdown-on-failure; band-scoped CF/DV rule translation for Insert/Delete Cells (fully-contained ranges move, partial overlaps documented as unchanged).
- **Performance**: whole-column formatting clamps style-only creation to the used range for unbounded selections (bounded selections unchanged; column-default styles rejected as no model/IO seam exists — recorded as the deeper follow-up); viewport row-header width computed from row metrics before cell materialization (no double build); GridView render caches persist across frames with a 512-entry trim (all six caches verified immutable-keyed, no theme/zoom invalidation needed); Autofill recalc uses `AffectedCells`; full-save styles applied once per distinct `StyleId` via cached `XLStyleValue` assignment.
- **Consolidations**: A1 parsing/formatting delegated to `CellAddress` everywhere (deleting two `checked`-overflow crash variants in the patch-save path; `WriteColumnName`/`GetColumnNameLength`/`GetRowDigitCount` promoted to public); `SetAttributeIfDifferent`/`RemoveAttributeIfPresent` unified in `XlsxXmlNormalizationHelpers`; `G17` formatting unified in `XlsxNumberFormatting.ToXmlString` (9 sites); cfvo mapping paired in `XlsxAdvancedConditionalFormatMetadata` (x14 autoMin/autoMax variant intentionally separate); `GridRange.TryIntersect`/`Contains(GridRange)` added and four private intersect copies deleted; per-class test save helpers unified in `XlsxPackageTestHelper`.
- **Architecture slices**: document dirty/save state extracted from MainWindow into the WPF-free `WorkbookDocumentState` service (App.Services, DI-registered per window; workflow re-entrancy guards deliberately stayed in MainWindow); the declarative sanitizer mechanism landed (`XlsxWorkbookLeafElementSchemas` table + generic single-pass normalizer) with eight workbook.xml leaf normalizers migrated to table rows — child-pruning/cross-element normalizers (definedNames, views, extLst, externalReferences, pivotCaches, oleSize, webPublishObjects, smartTagTypes) documented as residual code pending table extensions.
- **Test architecture**: new `FreeX.App.Host.Logic.Tests` project moved 149 pure planner/logic test files (1,411 tests) from the UI lane into the default gating lane; central package management adopted (`Directory.Packages.props`, all csproj versions stripped, no skew found).

Deliberately deferred (with reasons): full MainWindow extraction and full sanitizer migration (incremental follow-ups on the seams now established); hygiene-test altitude (converting ~40 string-asserting test files to reflection/Roslyn checks is a convention change best done incrementally as files are touched, not a one-shot rewrite); Excel-style column-default styles (needs a model+IO+render seam first).

Verification on integrated `main` after the second pass: repository preflight passed, `FreeX.slnx` Release build 0 warnings/0 errors, default test lane 12,463 passed / 1 failed across seven projects — the single failure is the pre-existing wall-clock-sensitive `RepeatedBooleanCoercionFormulaTextEvaluation_AvoidsCoercedNumberChurn` allocation test, which fails only under full-suite parallel CPU load and passes in isolation (verified); aligning it with the trait-gated benchmark convention used by its siblings is a candidate follow-up.
