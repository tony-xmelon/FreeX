...
| Workbook theme (load/save; cell-style color resolution; chart/shape rendering) | Partial | Loaded `fmtScheme` XML is preserved and the first `outerShdw` effect style is interpreted for non-chart drawing-object theme shadows; deeper OOXML effects remain deferred |
