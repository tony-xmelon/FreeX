...
| Workbook theme (load/save; cell-style color resolution; chart/shape rendering) | Partial | Loaded `fmtScheme` XML is preserved and the first supported effect style is interpreted for bounded non-chart drawing-object theme outer shadows, inner shadows, glow, and soft edges; deeper OOXML effects remain deferred |
| Conditional formatting (icon sets) | Partial | Model scaffold; rendering partial |
| Advanced chart families (surface/treemap/waterfall/etc.) | Partial | Combo/radar/stock, 2D/3D surface, histogram/Pareto, waterfall, treemap, sunburst, box-whisker, and funnel charts have package and rendering paths; the final 2026-06-01 chart harness passed 28/28 openability/export and visual gates, with the chartEx subset passing 7/7; map charts are recognized as deferred models when their package parts are parseable, while unparseable or unsupported chart parts still use the unsupported-feature warning path; deeper authoring/editing polish and richer formatting remain partial |
...
- Map/unparseable chart parts and unsupported or richer chart formatting
- Deeper OOXML effect semantics beyond the current first-supported `outerShdw`/`prstShdw`/`innerShdw`/`glow`/`softEdge` non-chart theme-effect slice, plus full XLSX chart-theme extraction beyond the native/XLSX workbook theme model, loaded-cell-style color-resolution, drawing-object theme-rendering/effect baseline, simple embedded native-chart package loading, chart theme-color rendering, preset menus, and custom theme dialog baseline
