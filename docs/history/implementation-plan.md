...
**Basis:** Gap analysis from parity/functions.md, parity/command-surface.md, parity/shortcuts.md, formats/fidelity-contract.md, and parity/menu-toolbar.md

> **Historical note (2026-05-19):** This plan is retained as implementation history. Its formula gap phases are complete and some scope decisions here have since changed (for example `CELL`, `INFO`, `LET`, `LAMBDA`, statistical distributions, financial functions, `HYPERLINK`, and engineering base/bit functions are implemented). Use `parity/functions.md`, `planning/next-phases.md`, and `planning/outstanding-build.md` for current status.
> **Status correction (2026-06-01):** The multi-window View advanced controls formerly listed below are implemented and should not be treated as excluded or deferred. Current command status is in `parity/command-surface.md`.
