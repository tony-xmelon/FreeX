...
The main operational risk is again coordination rather than a single missing feature. The workspace currently has many active worktrees and local branches, including several dirty implementation branches. Treat `main`/`origin/main` plus this report, [planning/outstanding-build.md](../planning/outstanding-build.md), [planning/next-phases.md](../planning/next-phases.md), [parity/command-surface.md](../parity/command-surface.md), [parity/shortcuts.md](../parity/shortcuts.md), [formats/fidelity-contract.md](../formats/fidelity-contract.md), and [formats/xlsx-corpus-report.md](../formats/xlsx-corpus-report.md) as the current documentation set.
...
This pass updates the documentation index, records a fresh status snapshot, and removes stale references to the May 23 report as the current status document. Historical `docs/archive/superpowers/plans/*` and `docs/archive/superpowers/specs/*` files remain useful implementation records but should not be treated as current build-status sources.

Known documentation cleanups still worth doing:

1. Split the very large [reviews/code-review-log.md](../reviews/code-review-log.md) into current findings plus an archive.
...
3. Keep the release-facing docs (`user/guide.md` and `user/troubleshooting.md`) plus the release notes workflow aligned with `main`.
4. Keep [planning/outstanding-build.md](../planning/outstanding-build.md) and [planning/next-phases.md](../planning/next-phases.md) aligned whenever active branches merge.
