...
| Gradients/Effects | Partial | Shape gradient fill with direction presets, dedicated access-keyed start/end color pickers, and selectable None/Shadow/Glow/Soft Edges effects with rendering plus native JSON/XLSX persistence; full Excel gradient/effect galleries pending |
...
| Flash Fill | Partial | Expanded deterministic inference including common first-name/last-name contact patterns, dotted/underscored/hyphenated email display-name cleanup with plus-address tag removal, plus-address email local-part extraction, email domain-stem extraction, web-address host/domain-stem cleanup from schemes, www prefixes, paths, queries, fragments, and bare hosts, digit-mask formatting such as phone-number punctuation, thousand-separator stripping, digit-only extraction, calendar-valid embedded-date extraction/normalization from labeled text with ambiguous multi-date sources rejected, US address component extraction such as street, city, state, ZIP, and ZIP+4 base ZIP, two-part full-name reordering, known title/suffix removal such as `Dr. Ada Lovelace`, `Dr. Lovelace`, `Dr. Ada Lovelace Jr., Ph.D.`, `Ada Lovelace Jr.`, or `Lovelace Jr.` to the untitled/unsuffixed name, uppercase initial inference from lowercase names, first/last or all-initial abbreviations such as `Ada Lovelace` to `Lovelace, Ada`, `A. Lovelace`, `A. L.`, `Ada L.`, `Lovelace A.`, or `Lovelace, A.`, exactly three-token name edge/middle-token drops, full last-name reordering, and initial abbreviations such as `Ada Byron Lovelace` to `Byron Lovelace`, `Ada Byron`, `Ada Lovelace`, `Lovelace, Ada`, `Lovelace, Ada Byron`, `Ada B. Lovelace`, `A. Lovelace`, `Ada L.`, `Lovelace A.`, `B. Lovelace`, `Byron L.`, `A. B. Lovelace`, `A. B. L.`, `Ada B.`, `Ada Byron L.`, `Lovelace, Ada B.`, `Lovelace A. B.`, `Lovelace, A. B.`, or `B.`, shared-domain email generation with `.`, `_`, or `-` first/last, first-initial/last, first/last-initial, and last/first-initial separators, and first/last-initial email aliases, single-cell invocation includes contiguous examples above the active cell, and selected ranges leave source-blank rows blank while still filling later populated source rows; Excel's full ML-like inference remains partial |
