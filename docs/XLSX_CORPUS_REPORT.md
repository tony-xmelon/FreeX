...
| Generated metadata-package Excel smoke generation | Active opt-in gate; the Excel smoke generator now materializes supported-metadata-pass package fixtures from the corpus manifest when requested with `--corpus-status supported-metadata-pass`. The latest real Excel save/reopen run passed 48/52 rows after slicer/timeline package graph and custom XML relationship fixes, leaving 4 remaining package/schema openability gaps: printer settings binary, workbook smart tags, worksheet single XML cells, and worksheet smart tags |
