...
| Supported manifest feature smoke expectations | Pass; all supported-pass, public-pass, and supported-pivot-metadata-pass manifest rows now derive minimum bidirectional smoke expectations from measurable feature tags, so Excel save/reopen runs assert tagged formulas, tables, validations, conditional formats, hyperlinks, comments, pictures, sparklines, text boxes, shapes, protected sheets, structure protection, PivotTables, and pivot caches instead of only checking openability |
| Generated metadata-package Excel smoke generation | Active opt-in gate; the Excel smoke generator now materializes supported-metadata-pass package fixtures from the corpus manifest when requested with `--corpus-status supported-metadata-pass`. The latest real Excel save/reopen run passed 45/52 rows after schema-valid native metadata fixture fixes, exposing 7 remaining package/schema openability gaps: printer settings binary, workbook/worksheet smart tags, worksheet single XML cells, slicer/timeline roots, and package-level custom XML relationships |
