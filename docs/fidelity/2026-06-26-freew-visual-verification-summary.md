...
Later status, 2026-07-02: the shared WPF/Avalonia visual evidence contract is now implemented in `FreeW.App.Presentation`, with WPF `FreeW.FidelityRender`, Avalonia `FreeW.PageLayoutShot`, and `FreeW.VisualEvidenceSummary` producing a common manifest/summary. The current smoke lane generates 15 F2/page-composition DOCX fixtures, WPF renders 32 PNGs, Avalonia renders 20 PNGs, and the combined summary reports 52 trusted evidence rows, including paired footnote/endnote placement, section geometry, table layout, drawing objects, and chart/SmartArt composition coverage. WPF emits true portrait/landscape page dimensions for `f2-section-landscape`; Avalonia emits the shared section ownership and expected page-geometry metadata, while its row metadata still marks the remaining renderer gap as `avalonia-global-page-surface-no-section-page-break`. Treat the sections below as the June 26 historical triage; current follow-up should use the shared Word-baseline comparison and tolerance reporting path to prioritize true Avalonia mixed-section page surfaces and WordArt/watermark stress.
