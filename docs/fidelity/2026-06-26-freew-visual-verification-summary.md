...
Later status, 2026-07-02: the shared WPF/Avalonia visual evidence contract is now implemented in `FreeW.App.Presentation`, with WPF `FreeW.FidelityRender`, Avalonia `FreeW.PageLayoutShot`, and `FreeW.VisualEvidenceSummary` producing a common manifest/summary. The current smoke lane generates 15 F2/page-composition DOCX fixtures, WPF renders 32 PNGs, Avalonia renders 18 PNGs, and the combined summary reports 50 trusted evidence rows, including footnote/endnote placement, table layout, drawing objects, and chart/SmartArt composition coverage. Treat the sections below as the June 26 historical triage; current follow-up should use the shared Word-baseline comparison and tolerance reporting path to prioritize section geometry and WordArt/watermark stress.
