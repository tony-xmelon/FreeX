...
**Status:** Current working backlog after the June 4 end-to-end performance pass, loaded-workbook patch-save slices, and deterministic XLSX IO benchmark fixture.
...
1. Fold more load-time package facts together: sheet XML layout, sanitizer requirements, style-only strip decisions, and feature inspection still share adjacent ZIP/XML scans.
2. Split XLSX save further into ClosedXML workbook emission, modeled feature writers, native metadata merge/post-processing, package validation, and stream/file replacement for fallback saves.
3. Extend package patching beyond existing literal and formula-cache values: true new cells, `ClearCell` removals, style changes, formula text/topology changes, dimensions, shared strings, comments, hyperlinks, and structured-table impacts still need explicit guards and tests before they can bypass ClosedXML.
