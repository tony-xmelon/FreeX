...
**PDF export reassessment (2026-06-03):** the partial PDF/XPS status now includes selectable/searchable PDF text overlays for fully visible embedded chart titles, X/Y axis titles including rotated Y-axis title metadata, legend entries, category tick labels, and data labels for classic embedded category charts. Still deferred for parity: vector chart graphics, value-axis tick labels and full chart text coverage, chart-sheet pagination, full drawing-object z-order fidelity, internal workbook PDF destinations, PDF/A, and tagged PDF.
