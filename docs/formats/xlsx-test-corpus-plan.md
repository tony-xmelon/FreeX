...
Current executable manifest baseline: 180 rows (124 generated, 25 public, 22 local-private, 9 regression).
...
| User-provided local workbooks | 22 | Must be approved for local testing; manifest stores only filename, feature tags, and anonymized notes unless user allows more detail. The manifest now includes 20 optional rows that are skipped until local files are supplied plus the user-approved COIN Tool performance workbook and Partner Dashboard workbook paths for local coverage. |
