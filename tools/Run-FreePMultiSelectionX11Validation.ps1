...
try {
    ...
    Invoke-External (Get-ToolPowerShellPath) $startArgs; $started = $true
    ...
    if ($started -and -not $KeepContainer) { Invoke-ToolProcess -FilePath (Get-ToolPowerShellPath) -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", $genericRunner, "-Action", "Stop", "-App", "FreeP", "-Port", "$Port", "-OutputDir", $resolvedOutputRoot) -WorkingDirectory $repoRoot; $started = $false }
