...
try {
    ...
    $after = Read-Geometry inspect-nested-text $fixturePath
    ...
    $session = Get-Content $sessionPath -Raw | ConvertFrom-Json
    $sessionDir = [string]$session.sessionDirectory
    & docker exec $containerName bash -lc "window_id=\$(xdotool search --onlyvisible --name 'FreeW' 2>/dev/null | tail -1); test -n \"\$window_id\"; xdotool windowactivate --sync \"\$window_id\"; sleep 1; scrot /work/nested-text-wave64/04-reopened.png"
    if ($LASTEXITCODE -ne 0) { throw "The saved nested text document did not reopen in the Linux harness." }
    $reopened = Read-Geometry inspect-nested-text $fixturePath
    if ($reopened['child-text'] -ne 'Nested leaf!') { throw "Reopened nested text mismatch: '$($reopened['child-text'])'." }

    $probe = Get-Content (Join-Path $sessionDir "nested-text-wave64/probe-results.json") -Raw | ConvertFrom-Json
    $manifest = [ordered]@{
        schemaVersion = 1
        ...
        evidence = [ordered]@{ session = $sessionDir; fixture = $fixturePath }
    }
