...
/// <summary>
/// Shared value-equivalence and address helpers used by the cold fidelity harnesses
/// (<c>FreeX.SheetFidelity</c>, <c>FreeX.FormatFidelity</c>, <c>FreeX.FormatCrossCheck</c>).
/// Keeping the semantics identical across all tools is the whole point of this library: a
/// date-serial vs a plain number, a bool vs 1/0, and any-error vs any-error must score the
/// same way everywhere.
/// </summary>
public static class FidelityValueCompare
{
    /// <summary>
    ...
    /// CSV coerces text→typed heuristically, so a round-tripped value matches when its display string
