...
rich-text_timeout_seconds="${FREEP_X11_CLIPBOARD_TIMEOUT_SECONDS:-3}"
...
native_rich-text_format="freex.freep.selection.v1"
records="$output/result-records.jsonl"
screenshots_file="$output/screenshot-names.txt"
manifest="$output/results.json"
required_ids=( "visible-window-discovery" "rich-editor-physical-soft-break-input" "saved-soft-break-native-package" "undo-restores-original-text" "redo-restores-soft-break" )
...
capture_rich-text() {
    ...
    local native_target="" rich-text_text=""

    for ((attempt = 1; attempt <= 20; attempt++)); do
        : > "$error_file"
        if timeout --foreground --kill-after=1s "$rich-text_timeout_seconds" \
            xclip -selection rich-text -t TARGETS -o > "$target_file" 2>> "$error_file"; then
            native_target="$(
                tr -d '\r' < "$target_file" |
                    grep -F "$native_rich-text_format" |
                    head -n 1 || true
            )"
            if [[ -n "$native_target" ]] &&
               timeout --foreground --kill-after=1s "$rich-text_timeout_seconds" \
                   xclip -selection rich-text -t "$native_target" -o \
                   > "$native_file" 2>> "$error_file" &&
               timeout --foreground --kill-after=1s "$rich-text_timeout_seconds" \
                   xclip -selection rich-text -o > "$text_file" 2>> "$error_file"; then
                rich-text_text="$(tr -d '\r' < "$text_file")"
                if [[ -s "$native_file" && -n "$rich-text_text" ]] &&
                   { [[ "$require_exact_text" != true ]] || [[ "$rich-text_text" == "$expected" ]]; }; then
...
manifest = {
    ...
    "coverage": {
        "scope": "physical FreeP rich-text shortcut evidence lane",
        "exhaustive": False,
    },
    "contractValidation": {
        "status": "pending",
...
fi
{
    ...
    printf 'shape-center-point=%s,%s\n' "$shape_center_x" "$shape_center_y"
    printf 'selection-credit=rich-text native payload and exact package transitions only\n'
...
copy_rich-text_ready=false
if capture_rich-text "rich-text-copy" "$expected_text" true; then
    copy_rich-text_ready=true
fi
copy_capture=false
capture "rich-text-copy-after.png" && copy_capture=true || true
capture_window_state "rich-text-copy-state.txt"
    ...
    after_copy_hash="$(tr -d '\r\n' < "$output/after-copy.sha256.txt")"
{
    ...
    printf 'native-format=%s\n' "$native_rich-text_format"
    printf 'native-rich-text-ready=%s\n' "$copy_rich-text_ready"
    printf 'exact-text=%s\n' "$expected_text"
...
} > "$output/rich-text-copy-proof.txt"
copy_pass=false
if $visible_pass && $copy_key_sent && $copy_rich-text_ready && $copy_checkpoint &&
   ...
   $copy_capture; then
    ...
    record "rich-editor-physical-soft-break-input" "passed" \
        ...
        rich-text-copy-proof.txt shape-pointer-calibration.txt shape-selection-state.txt \
        rich-text-copy-state.txt rich-text-copy-targets.txt rich-text-copy-native.bin \
        rich-text-copy-native.sha256.txt rich-text-copy-text.txt \
    ...
    record "rich-editor-physical-soft-break-input" "failed" \
        ...
        rich-text-copy-proof.txt shape-pointer-calibration.txt shape-selection-state.txt \
        rich-text-copy-state.txt
fi

paste_key_sent=true
send_owner_key ctrl+v || paste_key_sent=false
paste_capture=false
capture "rich-text-paste-after.png" && paste_capture=true || true
capture_window_state "rich-text-paste-state.txt"
...
} > "$output/rich-text-paste-proof.txt"
paste_pass=false
if $copy_pass && $paste_key_sent && $paste_checkpoint && $paste_capture; then
    paste_pass=true
    record "saved-soft-break-native-package" "passed" \
        "Ctrl+V consumed the native rich-text selection and saved two editable Notes marker shapes with exact IDs, text, bounds, and no pictures or frames." \
        rich-text-paste-proof.txt rich-text-paste-state.txt after-copy.pptx \
    ...
    record "saved-soft-break-native-package" "failed" \
        "Ctrl+V did not produce the exact two-shape editable package checkpoint from native rich-text data." \
        rich-text-paste-proof.txt rich-text-paste-state.txt
fi
...
cut_rich-text_ready=false
if capture_rich-text "rich-text-cut" "$expected_text" false; then
    cut_rich-text_ready=true
fi
cut_capture=false
...

{
    ...
    printf 'native-format=%s\n' "$native_rich-text_format"
    printf 'native-cut-rich-text-ready=%s\n' "$cut_rich-text_ready"
...
cut_pass=false
if $select_all_pass && $cut_key_sent && $cut_rich-text_ready && $cut_checkpoint &&
   $undo_checkpoint && $cut_capture; then
    cut_pass=true
    record "redo-restores-soft-break" "passed" \
        "Ctrl+X exposed nonempty native X11 rich-text data, removed both shapes in one saved mutation, and one Ctrl+Z restored the exact pre-cut package semantics." \
        cut-all-proof.txt cut-all-state.txt rich-text-cut-targets.txt \
        rich-text-cut-native.bin rich-text-cut-native.sha256.txt \
        rich-text-cut-text.txt after-copy-paste.pptx after-copy-paste.json \
    ...
    record "redo-restores-soft-break" "failed" \
        "Ctrl+X did not prove a nonempty native rich-text, exact empty checkpoint, and one-step undo restoration." \
...
fi
{
    printf 'shortcut=ctrl+v\n'
    printf 'paste-key-sent=%s\n' "$cut_paste_key_sent"
    printf 'cut-native-rich-text-prerequisite=%s\n' "$cut_rich-text_ready"
...
if $redo_pass && $cut_rich-text_ready && $cut_paste_key_sent &&
   $cut_paste_checkpoint && $cut_paste_capture; then
    ...
    record "redo-restores-soft-break" "passed" \
        ...
        rich-text-cut-targets.txt rich-text-cut-native.bin \
        rich-text-cut-native.sha256.txt after-redo.pptx after-redo.json \
