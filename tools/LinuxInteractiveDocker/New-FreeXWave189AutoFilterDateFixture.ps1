...
function New-DateCell {
    ...
    '<c r="' + $Address + '" s="1"><v>' +
        $Value.ToOADate().ToString([Globalization.CultureInfo]::InvariantCulture) + '</v></c>'
}
...
$temporary = Join-Path $parent ".freex-wave189-$PID-$([guid]::NewGuid().ToString('N').Substring(0, 8)).tmp"
...
$workbook = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="AutoFilter Date" sheetId="1" r:id="rId1"/></sheets><calcPr calcId="191029" calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1"/></workbook>'
...
$styles = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="1"><numFmt numFmtId="164" formatCode="yyyy-mm-dd"/></numFmts><fonts count="1"><font><sz val="11"/><name val="Calibri"/></font></fonts><fills count="2"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="2"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="164" fontId="0" fillId="0" borderId="0" xfId="0"/></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>'

$rows = @(
    @("Region", $null),
    @("Jan01", [datetime]::new(2024, 1, 1)),
    @("Jan15", [datetime]::new(2024, 1, 15)),
    @("Boundary", [datetime]::new(2024, 2, 1)),
    @("Mar15", [datetime]::new(2024, 3, 15))
)
...
for ($row = 0; $row -lt $rows.Count; $row++) {
    ...
    if ($null -eq $rows[$row][1]) {
        [void]$rowXml.Append((New-InlineStringCell -Address "B$number" -Value "Date"))
    } else {
        [void]$rowXml.Append((New-DateCell -Address "B$number" -Value $rows[$row][1]))
    }
