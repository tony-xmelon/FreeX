...
$temporary = Join-Path $parent ".freex-wave195-multi-$PID-$([guid]::NewGuid().ToString('N').Substring(0, 8)).tmp"
...
$workbook = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="AutoFilter Multi" sheetId="1" r:id="rId1"/></sheets><calcPr calcId="195029" calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1"/></workbook>'
...
$styles = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="1"><font><sz val="11"/><name val="Calibri"/></font></fonts><fills count="2"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/></cellStyleXfs><cellXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>'
...
for ($row = 0; $row -lt $rows.Count; $row++) {
    ...
    for ($column = 0; $column -lt 3; $column++) {
        $address = [char]([int][char]'A' + $column) + [string]$number
        if ($column -lt 2) {
            [void]$rowXml.Append((New-InlineStringCell -Address $address -Value $rows[$row][$column]))
        } else {
            [void]$rowXml.Append('<c r="' + $address + '"><v>' + $rows[$row][$column] + '</v></c>')
        }
    }
    [void]$rowXml.Append('</row>')
}
$worksheet = "<?xml version=`"1.0`" encoding=`"UTF-8`" standalone=`"yes`"?><worksheet xmlns=`"http://schemas.openxmlformats.org/spreadsheetml/2006/main`"><dimension ref=`"A1:C7`"/><sheetViews><sheetView workbookViewId=`"0`" showGridLines=`"1`"/></sheetViews><sheetData>$($rowXml.ToString())</sheetData><autoFilter ref=`"A1:C7`"/></worksheet>"
