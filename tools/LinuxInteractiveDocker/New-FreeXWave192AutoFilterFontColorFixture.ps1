...
function New-StyledInlineStringCell {
    param(
...
$temporary = Join-Path $parent ".freex-wave192-$PID-$([guid]::NewGuid().ToString('N').Substring(0, 8)).tmp"
...
$workbook = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="AutoFilter Font Color" sheetId="1" r:id="rId1"/></sheets><calcPr calcId="192029" calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1"/></workbook>'
...
$styles = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Calibri"/></font><font><sz val="11"/><name val="Calibri"/><color rgb="FF00B050"/></font></fonts><fills count="2"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/></cellStyleXfs><cellXfs count="2"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="0" borderId="0" xfId="0"/></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>'
...
for ($row = 0; $row -lt $rows.Count; $row++) {
    ...
    $cell = if ($rows[$row][1] -eq 0) {
        New-InlineStringCell -Address "A$number" -Value $rows[$row][0]
    } else {
        New-StyledInlineStringCell -Address "A$number" -Value $rows[$row][0] -StyleId $rows[$row][1]
    }
    [void]$rowXml.Append($cell)
    [void]$rowXml.Append((New-InlineStringCell -Address "B$number" -Value "Value"))
