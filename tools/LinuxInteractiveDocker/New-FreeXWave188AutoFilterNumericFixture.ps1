...
function New-NumberCell {
    param([Parameter(Mandatory = $true)][string]$Address, [Parameter(Mandatory = $true)][double]$Value)
    '<c r="' + $Address + '"><v>' + $Value.ToString([Globalization.CultureInfo]::InvariantCulture) + '</v></c>'
}
...
$temporary = Join-Path $parent ".freex-wave188-$PID-$([guid]::NewGuid().ToString('N').Substring(0, 8)).tmp"
...
$workbook = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="AutoFilter Numeric" sheetId="1" r:id="rId1"/></sheets><calcPr calcId="191029" calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1"/></workbook>'
...
for ($row = 0; $row -lt $rows.Count; $row++) {
    ...
    if ($null -eq $rows[$row][1]) {
        ...
        [void]$rowXml.Append((New-NumberCell -Address "B$number" -Value ([double]$rows[$row][1])))
    }
