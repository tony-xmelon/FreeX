...
try {
    ...
    $freeP = [ordered]@{
        ...
        nextSlice = "Layout picker, table picker, rich inline table/text editing, slide-pane reorder, bottom New Slide, notes-page preview/PDF rendering, presenter recording/ink execution with repeated custom-show route retention, modern comments/review, and accessibility/proofing workflow depth now have WPF/Avalonia no-COM evidence paths. Continue workflow-depth slices in real narration/camera capture backends, richer visible list-gallery/image-bullet UI, and PowerPoint/package baselines."
    }
