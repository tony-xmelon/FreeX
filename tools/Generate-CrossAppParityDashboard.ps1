...
try {
    ...
    $freeP = [ordered]@{
        ...
        nextSlice = "Layout picker, table picker, rich inline table/text editing, shared visible list galleries, picture-bullet import/rendering plus picker/media authoring, slide-pane reorder, bottom New Slide, notes-page preview/PDF rendering, fixed-layout PDF connector, image, triangle-arrowhead, and rotation export, export/backstage package-handoff, presenter recording backend contracts plus capture-adapter readiness and paired backend injection, captured PPTX media and generated WebVTT caption artifact authoring, focused PowerPoint-native caption package retention including external caption relationship metadata, shared chart number/date label rendering plus edge/mixed manual-layout bounds and surface grid rendering, SmartArt basic hierarchy plus vertical bullet list plus basic cycle plus radial cycle plus gear cycle plus text cycle plus basic, continuous block, segmented, and chevron process plus basic block list plus vertical box list plus stacked list plus bounded pictureCaptionList live layout plus shared outline reorder/promote/demote editing, header/footer placeholder creation plus inherited layout/master geometry, title-slide suppression, fixed/auto date options, shared OMML matrix spacing/base alignment, delimiter grow semantics, accent/bar render-plan coverage, manual line breaks, boxed equation-array alignment points, and transparent phantom spacing classes, ink execution with repeated custom-show route retention, modern comments/review with shared mention candidate and insertion planning, accessibility/proofing workflow depth, and animation-pane row/playback workflow evidence now have WPF/Avalonia no-COM evidence paths. Continue workflow-depth slices in real OS microphone/camera implementations, chart visual baselines and remaining type-specific chart decisions, true contour/3-D surface chart geometry, remaining SmartArt layout geometry and deeper authoring/editing, remaining OMML structure/layout and PowerPoint-authoritative math visual-baseline coverage, broader PowerPoint-native media/caption package baselines, broader PowerPoint-authoritative PDF visual baselines, and PowerPoint-authoritative animation-pane visual/playback baselines."
      }
