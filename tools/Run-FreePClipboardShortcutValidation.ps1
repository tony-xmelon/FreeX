...
try {
    ...
    if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
        ...
        foreach ($entry in $hashPaths.GetEnumerator()) { if (-not (Test-Path -LiteralPath $entry.Value -PathType Leaf)) { $hashFailures.Add("$($entry.Key) hash artifact is missing"); continue }; $value = (Get-Content -LiteralPath $entry.Value -Raw).Trim(); if ($value -notmatch '^[0-9a-f]{64}$') { $hashFailures.Add("$($entry.Key) hash is not an exact lowercase 64-hex value"); continue }; $hashes[$entry.Key] = $value }
        foreach ($pair in @(@("source-before", "source-after"), @("source-before", "mounted-before"), @("source-before", "mounted-after"), @("mounted-after", "host-mounted-after"))) { if ($hashes.ContainsKey($pair[0]) -and $hashes.ContainsKey($pair[1]) -and $hashes[$pair[0]] -ne $hashes[$pair[1]]) { $hashFailures.Add("$($pair[0]) does not equal $($pair[1])") } }
        if ($hashFailures.Count -gt 0) { $copyResult.status = "failed"; $copyResult.note = "Clipboard fixture SHA256 evidence failed: $([string]::Join('; ', $hashFailures))." }
