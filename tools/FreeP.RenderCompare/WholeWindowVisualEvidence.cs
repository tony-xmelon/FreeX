...
internal static class WholeWindowVisualEvidence
{
        ...
        IReadOnlyList<string>? avaloniaDuplicatePeers)
    {
        ...
        if (wpf.SemanticState.AppOwnedTitleBarVisible && wpfTitleBarRaster?.IsValid != true)
            Mismatch("app-owned-titlebar-raster", $"WPF titlebar surface is not visibly present in its declared raster bounds (accent ratio {wpfTitleBarRaster?.AccentPixelRatio ?? 0:P1}; neutral caption ratio {wpfTitleBarRaster?.NeutralCaptionPixelRatio ?? 0:P1}).");
        if (avalonia.SemanticState.AppOwnedTitleBarVisible && avaloniaTitleBarRaster?.IsValid != true)
            Mismatch("app-owned-titlebar-raster", $"Avalonia titlebar surface is not visibly present in its declared raster bounds (accent ratio {avaloniaTitleBarRaster?.AccentPixelRatio ?? 0:P1}; neutral caption ratio {avaloniaTitleBarRaster?.NeutralCaptionPixelRatio ?? 0:P1}).");
