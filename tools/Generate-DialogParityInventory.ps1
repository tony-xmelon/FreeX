...
function Get-AvaloniaParityDialogIds {
    $sourcePath = Join-Path $repoRoot "src\FreeX.App.Avalonia\MainWindow.ParityCapture.cs"
...
[void]$md.AppendLine()
[void]$md.AppendLine("Generated by tools/Generate-DialogParityInventory.ps1 from current repo files at commit $sourceCommit.")
[void]$md.AppendLine()
[void]$md.AppendLine("This is an inventory slice, not a parity claim. WPF capture and Avalonia capture mean a current checked-in PNG or capture-asset manifest entry was found. Avalonia harness means the route is present in src/FreeX.App.Avalonia/MainWindow.ParityCapture.cs, even when no committed capture asset exists.")
[void]$md.AppendLine()
