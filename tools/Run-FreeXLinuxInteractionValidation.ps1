...
$authoritativeRibbonBindingRowCount = 654
$authoritativeCollapsedRibbonGroupRowCount = 76
...
function Assert-AutoFilterFontColorPostcondition {
    param([Parameter(Mandatory = $true)][string]$EvidenceDirectory)

    $gatePath = Join-Path $EvidenceDirectory "autofilter-font-color-swatch-gate.txt"
    $postconditionPath = Join-Path $EvidenceDirectory "autofilter-font-color-postcondition.txt"
    if (-not (Test-Path -LiteralPath $gatePath -PathType Leaf) -or
        -not (Test-Path -LiteralPath $postconditionPath -PathType Leaf)) {
        throw "AutoFilter font-color probe did not emit its rendered-swatch gate and postcondition."
    }

    $gateLines = @(Get-Content -LiteralPath $gatePath)
    $expectedGate = @(
        "schema-version=1",
        "before=autofilter-font-color-before.png",
        "source=autofilter-font-color-menu-open.png",
        "mode=font",
        "button-bounds=97,439,75,27",
        "click=139,456",
        "sample=113,452",
    ...
    $missingGate = @($expectedGate | Where-Object { $_ -notin $gateLines })
    if ($missingGate.Count -ne 0) {
        throw "AutoFilter font-color rendered-swatch gate failed: $($missingGate -join '; ')."
    }
    ...
    $missingPostconditions = @($expectedPostconditions | Where-Object { $_ -notin $postconditionLines })
    if ($missingPostconditions.Count -ne 0) {
        throw "AutoFilter font-color postcondition failed exact rendered/semantic/package contract: $($missingPostconditions -join '; ')."
    }
}
...
try {
    ...
    if ($PhysicalProbeSelector -eq "ribbon-formatting") {
        if ([string]::IsNullOrWhiteSpace($PhysicalDocumentPath)) {
            $PhysicalDocumentPath = Join-Path $reportDirectory "fixtures/freex-wave196-ribbon-formatting.xlsx"
            & $ribbonFormattingFixtureGenerator -OutputPath $PhysicalDocumentPath
        }
        if (-not (Test-Path -LiteralPath $PhysicalDocumentPath -PathType Leaf) -or
            [IO.Path]::GetExtension($PhysicalDocumentPath) -ine ".xlsx") {
            throw "ribbon-formatting requires an existing .xlsx PhysicalDocumentPath."
        }
    }
    if ($PhysicalProbeSelector -eq "ribbon-number-format") {
        if ([string]::IsNullOrWhiteSpace($PhysicalDocumentPath)) {
            $PhysicalDocumentPath = Join-Path $reportDirectory "fixtures/freex-wave197-ribbon-number-format.xlsx"
            & $ribbonNumberFormatFixtureGenerator -OutputPath $PhysicalDocumentPath
        }
        if (-not (Test-Path -LiteralPath $PhysicalDocumentPath -PathType Leaf) -or
            [IO.Path]::GetExtension($PhysicalDocumentPath) -ine ".xlsx") {
    ...
    if ($PhysicalProbeSelector -eq "autofilter-sort-persistence") {
        if ([string]::IsNullOrWhiteSpace($PhysicalDocumentPath)) {
            $PhysicalDocumentPath = Join-Path $reportDirectory "fixtures/freex-wave185-autofilter-sort.xlsx"
            & $autoFilterSortFixtureGenerator -OutputPath $PhysicalDocumentPath
        }
        if (-not (Test-Path -LiteralPath $PhysicalDocumentPath -PathType Leaf) -or
            [IO.Path]::GetExtension($PhysicalDocumentPath) -ine ".xlsx") {
            throw "autofilter-sort-persistence requires an existing .xlsx PhysicalDocumentPath."
        }
    }
    if ($PhysicalProbeSelector -eq "autofilter-text-criteria-persistence") {
        if ([string]::IsNullOrWhiteSpace($PhysicalDocumentPath)) {
            $PhysicalDocumentPath = Join-Path $reportDirectory "fixtures/freex-wave186-autofilter-text.xlsx"
            & $autoFilterTextFixtureGenerator -OutputPath $PhysicalDocumentPath
        }
        if (-not (Test-Path -LiteralPath $PhysicalDocumentPath -PathType Leaf) -or
            [IO.Path]::GetExtension($PhysicalDocumentPath) -ine ".xlsx") {
            throw "autofilter-text-criteria-persistence requires an existing .xlsx PhysicalDocumentPath."
        }
    }
    if ($PhysicalProbeSelector -eq "autofilter-numeric-criteria-persistence") {
        if ([string]::IsNullOrWhiteSpace($PhysicalDocumentPath)) {
            $PhysicalDocumentPath = Join-Path $reportDirectory "fixtures/freex-wave188-autofilter-numeric.xlsx"
            & $autoFilterNumericFixtureGenerator -OutputPath $PhysicalDocumentPath
        }
        if (-not (Test-Path -LiteralPath $PhysicalDocumentPath -PathType Leaf) -or
            [IO.Path]::GetExtension($PhysicalDocumentPath) -ine ".xlsx") {
    ...
    if ($PhysicalProbeSelector -eq "autofilter-color-persistence") {
        if ([string]::IsNullOrWhiteSpace($PhysicalDocumentPath)) {
            $PhysicalDocumentPath = Join-Path $reportDirectory "fixtures/freex-wave191-autofilter-color.xlsx"
            & $autoFilterColorFixtureGenerator -OutputPath $PhysicalDocumentPath
        }
        if (-not (Test-Path -LiteralPath $PhysicalDocumentPath -PathType Leaf) -or
            [IO.Path]::GetExtension($PhysicalDocumentPath) -ine ".xlsx") {
            throw "autofilter-color-persistence requires an existing .xlsx PhysicalDocumentPath."
        }
    }
    if ($PhysicalProbeSelector -eq "autofilter-font-color-persistence") {
        if ([string]::IsNullOrWhiteSpace($PhysicalDocumentPath)) {
            $PhysicalDocumentPath = Join-Path $reportDirectory "fixtures/freex-wave192-autofilter-font-color.xlsx"
            & $autoFilterFontColorFixtureGenerator -OutputPath $PhysicalDocumentPath
        }
        if (-not (Test-Path -LiteralPath $PhysicalDocumentPath -PathType Leaf) -or
            [IO.Path]::GetExtension($PhysicalDocumentPath) -ine ".xlsx") {
    ...
    } elseif ($PhysicalProbeSelector -eq "ribbon-formatting") {
    ...
    } elseif ($PhysicalProbeSelector -eq "autofilter-text-criteria-persistence") {
    ...
    } elseif ($PhysicalProbeSelector -eq "autofilter-color-persistence") {
    ...
    } elseif ($PhysicalProbeSelector -eq "autofilter-sort-persistence") {
        @("autofilter-sort-ascending-descending-save-reopen-physical")
    } elseif ($PhysicalProbeSelector -eq "autofilter-text-criteria-persistence") {
        @("autofilter-text-criteria-begins-with-save-reopen-physical", "autofilter-text-criteria-equals-save-reopen-physical")
    } elseif ($PhysicalProbeSelector -eq "autofilter-numeric-criteria-persistence") {
        @("autofilter-numeric-criteria-greater-than-save-reopen-physical", "autofilter-numeric-criteria-equals-save-reopen-physical")
