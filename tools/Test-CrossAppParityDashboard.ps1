...
$acceptanceRefreshTestedSourceCommit = "729172266c0f054e37abd3fc3535c0e2f2c87652"
...
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.fullReleaseBuild -eq "Passed at tested source commit ${acceptanceRefreshTestedSourceCommit}: dotnet build FreeX.slnx --configuration Release -m:1 passed with 0 warnings and 0 errors; elapsed 00:06:07.") "Wave194 Release-build evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.defaultNonUiTestLane -eq "Passed at tested source commit ${acceptanceRefreshTestedSourceCommit}: final default non-UI lane produced 31 unique TRXs and matching console aggregation: 43,465 passed, 134 intentional skips, 0 failed, 43,599 total.") "Wave194 default-lane evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.initialDefaultLane -match "43,465 passed, 134 intentional skips, 0 failed, 43,599 total") "Wave194 current default-lane result must be recorded exactly."
