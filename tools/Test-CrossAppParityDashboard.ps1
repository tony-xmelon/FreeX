...
$acceptanceRefreshTestedSourceCommit = "8624e6d1f4bce133a3685d99f366e668491ea33f"
...
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.repositoryPreflight -eq "Passed at tested source commit ${acceptanceRefreshTestedSourceCommit}: powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools\Test-RepositoryPreflight.ps1 exited 0; 294 JSON, 309 XML-backed, 125 PowerShell scripts, 10 test gates/48 assigned projects, 13,922 conflict-marker files checked, and all generated docs/evidence current.") "Wave194 repository-preflight evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.fullReleaseBuild -eq "Passed at tested source commit ${acceptanceRefreshTestedSourceCommit}: dotnet build FreeX.slnx --configuration Release -m:1 passed with 0 warnings and 0 errors; elapsed 00:06:14.37.") "Wave194 Release-build evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.defaultNonUiTestLane -eq "Passed at tested source commit ${acceptanceRefreshTestedSourceCommit}: final default non-UI lane produced 31 unique TRXs and matching console aggregation: 43,485 passed, 134 intentional skips, 0 failed, 43,619 total.") "Wave194 default-lane evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.initialDefaultLane -match "43,485 passed, 134 intentional skips, 0 failed, 43,619 total") "Wave194 current default-lane result must be recorded exactly."
