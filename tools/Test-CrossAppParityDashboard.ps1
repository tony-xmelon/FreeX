...
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.independentReviewStatus -eq "passed") "Wave194 independent acceptance review must be recorded as passed after the clean recheck."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.independentReview -eq "Passed: the clean independent final acceptance review rechecked tested source commit ${acceptanceRefreshTestedSourceCommit} after remediation of both P3 acceptance findings (Desktop PowerShell host determinism and elapsed-time provenance); no remaining acceptance findings were identified. This review does not alter the tested-source boundary, counts, timings, or visual claim boundaries.") "Wave194 independent acceptance review must cite the clean recheck and both remediated P3 findings without changing evidence boundaries."
