...
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.independentReviewStatus -eq "pending") "Wave194 independent acceptance review must remain pending until later review."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.independentReview -eq "Pending: an independent final cross-app acceptance review of tested source commit ${acceptanceRefreshTestedSourceCommit} remains outstanding and must be completed later; this refresh does not claim that review passed. This status does not alter the tested-source boundary, counts, timings, or visual claim boundaries.") "Wave194 independent acceptance review must remain explicitly pending without changing evidence boundaries."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.repositoryPreflight -eq "Passed at tested source commit ${acceptanceRefreshTestedSourceCommit}: powershell.exe -NoProfile -ExecutionPolicy Bypass -File tools\Test-RepositoryPreflight.ps1 exited 0; 294 JSON, 310 XML-backed, 126 PowerShell scripts, 11 GitHub workflows, 12 test gates/48 assigned projects, 13,994 conflict-marker files checked, and all generated docs/evidence current; elapsed 00:03:23.3371196.") "Wave194 repository-preflight evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.fullReleaseBuildMsBuildElapsed -eq "00:13:46.21") "Wave194 Release-build MSBuild elapsed evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.fullReleaseBuildWrapperElapsed -eq "00:13:46.4785808") "Wave194 Release-build wrapper elapsed evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.fullReleaseBuild -eq "Passed at tested source commit ${acceptanceRefreshTestedSourceCommit}: dotnet build FreeX.slnx --configuration Release -m:1 passed with 0 warnings and 0 errors; MSBuild-retained Time Elapsed 00:13:46.21; wrapper stopwatch 00:13:46.4785808.") "Wave194 Release-build evidence must distinguish MSBuild and wrapper elapsed times."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.defaultNonUiTestLaneWrapperElapsed -eq "00:17:37.4967641") "Wave194 default-lane wrapper elapsed evidence must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.defaultNonUiTestLaneTrxTimestampSpan -eq "12:41:31.975 to 12:59:08.603 (+03:00)") "Wave194 default-lane TRX timestamp span must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.defaultNonUiTestLaneTrxDuration -eq "00:17:36.628") "Wave194 default-lane TRX duration must be exact."
Assert-DashboardCondition ([string]$dashboard.integrationGateEvidence.defaultNonUiTestLane -eq "Passed at tested source commit ${acceptanceRefreshTestedSourceCommit}: final default non-UI lane produced 31 unique TRXs and matching console aggregation: 43,548 passed, 134 intentional skips, 0 failed, 43,682 total; wrapper stopwatch 00:17:37.4967641; independently parsed 31-TRX timestamp span 12:41:31.975 to 12:59:08.603 (+03:00); duration 00:17:36.628.") "Wave194 default-lane evidence must distinguish wrapper and TRX elapsed times."
