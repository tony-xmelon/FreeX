...
if ($setupGuardOffset -lt 0 -or -not $clearCaptureCall.Success -or $clearCaptureCall.Index -le $setupGuardOffset) {
    throw "FreeP PowerPoint chrome capture must preserve existing evidence until foreground setup passes."
}
if ($captureSource.IndexOf("retained the previous complete reference set", [StringComparison]::Ordinal) -lt 0) {
    throw "FreeP PowerPoint chrome capture must retain a complete prior reference set when foreground preflight is blocked."
}
