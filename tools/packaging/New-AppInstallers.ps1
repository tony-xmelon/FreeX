...
function New-WindowsInstaller {
    ...
    if ($Suite) {
        ...
        $lines.Add('  Parameters, TestInstallRoot: String;')
        $lines.Add('begin')
        $lines.Add('  Parameters := ''/VERYSILENT /CURRENTUSER /SUPPRESSMSGBOXES /NORESTART'';')
        $lines.Add('  TestInstallRoot := ExpandConstant(''{param:TestInstallRoot|}'');')
        $lines.Add('  if TestInstallRoot <> '''' then')
        $lines.Add('    Parameters := Parameters + '' /DIR="'' + AddBackslash(TestInstallRoot) + DisplayName + ''"'';')
        $lines.Add('  if not Exec(ExpandConstant(''{tmp}\'' + FileName), Parameters, '''', SW_HIDE, ewWaitUntilTerminated, ResultCode) then')
