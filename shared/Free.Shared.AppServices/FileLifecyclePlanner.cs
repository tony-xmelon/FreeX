...
/// <summary>
/// Builds the format-filter / default-extension strings for the native dialogs from per-app format
/// descriptors. Pure string composition — no dialog, no I/O — so the same builder serves every host.
/// </summary>
...
/// <summary>
/// Composes Windows-style <c>"Label (*.ext)|*.ext|…|All files (*.*)|*.*"</c> filter strings and the
/// matching default extension from a list of <see cref="FileFormatChoice"/>. Neutral and reusable;
/// the "All files" sentinel is conventional across desktop hosts.
/// </summary>
public static class FileDialogFilter
{
    /// <summary>The trailing "all files" filter segment appended to open/save filters.</summary>
    public const string AllFilesSegment = "All files (*.*)|*.*";
