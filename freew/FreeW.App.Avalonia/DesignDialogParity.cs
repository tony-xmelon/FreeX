...
/// <summary>Avalonia counterpart of WPF's Custom Paragraph Spacing dialog.</summary>
public sealed class CustomParagraphSpacingDialog : Window
{
    ...
    public CustomParagraphSpacingDialog(DocumentParagraphSpacingSet? current)
    {
        ...
        _before = MakeTextBox(state.SpaceBeforeText);
        _after = MakeTextBox(state.SpaceAfterText);
        _line = MakeTextBox(state.LineSpacingText);
        ...
        grid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        InsertDialogLayout.AddLabeledRow(grid, 0, "Space before (pt):", _before);
        InsertDialogLayout.AddLabeledRow(grid, 1, "Space after (pt):", _after);
        InsertDialogLayout.AddLabeledRow(grid, 2, "Line spacing (x):", _line);
        content.Children.Add(grid);
        AvaloniaCompactDialogChrome.ApplyValidationStatus(_status, InsertDialogLayout.ChromeStyle, new Thickness(0, 8, 0, 0));
        content.Children.Add(_status);
        content.Children.Add(CreateActionRow());
    ...
    private bool Accept(bool closeOnSuccess)
    {
        if (!CustomParagraphSpacingDialogPlanner.TryBuildResult(
                ...
                out var validation))
        {
            _status.Text = validation?.Message ?? CustomParagraphSpacingDialogPlanner.LineSpacingValidationMessage;
            _status.IsVisible = true;
            (validation?.Field switch
            {
    ...
    {
        var box = new TextBox { Text = value, MinWidth = 130 };
        AvaloniaCompactDialogChrome.ApplyTextBox(box, InsertDialogLayout.ChromeStyle);
        return box;
    }
}
