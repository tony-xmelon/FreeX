#!/usr/bin/env bash
# Build a Debian package (.deb) for FreeW from a self-contained .NET publish directory,
# alongside the relocatable tarball and AppImage. Requires dpkg-deb (Linux).
#
# Usage:
...
pkg_root="$output/freew_${version}_${deb_arch}"

rm -rf "$pkg_root"
mkdir -p "$pkg_root/DEBIAN"
mkdir -p "$pkg_root/usr/lib/freew"
mkdir -p "$pkg_root/usr/bin"
mkdir -p "$pkg_root/usr/share/applications"
mkdir -p "$pkg_root/usr/share/icons/hicolor/scalable/apps"
mkdir -p "$pkg_root/usr/share/metainfo"

cp -a "$published/." "$pkg_root/usr/lib/freew/"
chmod +x "$pkg_root/usr/lib/freew/FreeW"

cat > "$pkg_root/usr/bin/freew" <<'LAUNCH'
#!/usr/bin/env bash
set -euo pipefail
exec /usr/lib/freew/FreeW "$@"
LAUNCH
chmod +x "$pkg_root/usr/bin/freew"
...
Maintainer: FreeW <noreply@github.com>
Installed-Size: $installed_size_kb
Depends: libc6, libstdc++6, libfontconfig1, libice6, libsm6
Description: FreeW word processor
 FreeW is a word processor. This package contains the self-contained Linux build
 of the FreeW Avalonia app; no system .NET runtime is required.
CONTROL
...
deb_path="$output/freew_${version}_${deb_arch}.deb"
rm -f "$deb_path"
