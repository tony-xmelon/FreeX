...
### Mailings
- **Mail merge**: `«Field»` placeholders, CSV data source, insert field, preview record (next/prev), Finish & Merge (records concatenated, page-broken), and Send E-mail Messages validation plus message-body draft handoff to the default mail client (FreeW never auto-sends).
