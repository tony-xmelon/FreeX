...
public sealed class DoNotTrackFormattingRoundTripTests
{
    ...
    {
        var document = new TextDocument { DoNotTrackFormatting = true };
        var expectedSettings = SettingsDocument(new XElement(W + "doNotTrackFormatting"));
    ...
    public void DefaultOff_OmitsSettingsPartAndReopensOff()
    {
        ...
        Read(bytes).DoNotTrackFormatting.Should().BeFalse();
    }
    ...
    {
        var sourceElement = new XElement(W + "doNotTrackFormatting",
        ...
        reopened.DoNotTrackFormatting.Should().Be(expected);

        var firstSave = Write(reopened);
        var expectedSettings = SettingsDocument(
            expected ? new XElement(W + "doNotTrackFormatting") : null);
        var firstSettings = ReadXml(firstSave, "word/settings.xml");

        XNode.DeepEquals(expectedSettings, firstSettings).Should().BeTrue(
            "expected {0} but found {1}", expectedSettings, firstSettings);
        Read(firstSave).DoNotTrackFormatting.Should().Be(expected);

        var secondSave = Write(Read(firstSave));

        XNode.DeepEquals(firstSettings, ReadXml(secondSave, "word/settings.xml")).Should().BeTrue();
        Read(secondSave).DoNotTrackFormatting.Should().Be(expected);
    }
