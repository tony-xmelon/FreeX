...
public sealed class HideSpellingErrorsRoundTripTests
{
    ...
    {
        var document = new TextDocument { HideSpellingErrors = true };
        var expectedSettings = SettingsDocument(new XElement(W + "hideSpellingErrors"));
    ...
    public void DefaultOff_OmitsSettingsPartAndReopensOff()
    {
        ...
        Read(bytes).HideSpellingErrors.Should().BeFalse();
    }
    ...
    {
        var sourceElement = new XElement(W + "hideSpellingErrors",
        ...
        reopened.HideSpellingErrors.Should().Be(expected);

        var firstSave = Write(reopened);
        var expectedSettings = SettingsDocument(
            expected ? new XElement(W + "hideSpellingErrors") : null);
        var firstSettings = ReadXml(firstSave, "word/settings.xml");

        XNode.DeepEquals(expectedSettings, firstSettings).Should().BeTrue(
            "expected {0} but found {1}", expectedSettings, firstSettings);
        Read(firstSave).HideSpellingErrors.Should().Be(expected);

        var secondSave = Write(Read(firstSave));

        XNode.DeepEquals(firstSettings, ReadXml(secondSave, "word/settings.xml")).Should().BeTrue();
        Read(secondSave).HideSpellingErrors.Should().Be(expected);
    }
    ...
    public void EnabledSetting_OverlaysAtCtSettingsSchemaPosition()
    {
        ...
        Read(saved).HideSpellingErrors.Should().BeTrue();
    }
