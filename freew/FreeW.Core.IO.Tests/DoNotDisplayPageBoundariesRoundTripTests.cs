...
public sealed class DoNotDisplayPageBoundariesRoundTripTests
{
    ...
    {
        var document = new TextDocument { DoNotDisplayPageBoundaries = true };
        var expectedSettings = SettingsDocument(new XElement(W + "doNotDisplayPageBoundaries"));
    ...
    public void DefaultOff_OmitsSettingsPartAndReopensOff()
    {
        ...
        Read(bytes).DoNotDisplayPageBoundaries.Should().BeFalse();
    }
    ...
    {
        var sourceElement = new XElement(W + "doNotDisplayPageBoundaries",
        ...
        reopened.DoNotDisplayPageBoundaries.Should().Be(expected);

        var firstSave = Write(reopened);
        var expectedSettings = SettingsDocument(expected ? new XElement(W + "doNotDisplayPageBoundaries") : null);
        var firstSettings = ReadXml(firstSave, "word/settings.xml");

        XNode.DeepEquals(expectedSettings, firstSettings).Should().BeTrue(
            "expected {0} but found {1}", expectedSettings, firstSettings);
        Read(firstSave).DoNotDisplayPageBoundaries.Should().Be(expected);

        var secondSave = Write(Read(firstSave));

        XNode.DeepEquals(firstSettings, ReadXml(secondSave, "word/settings.xml")).Should().BeTrue();
        Read(secondSave).DoNotDisplayPageBoundaries.Should().Be(expected);
    }
    ...
    public void EnabledSetting_OverlaysAtCtSettingsSchemaPosition()
    {
        var source = BuildWordAuthoredPackage(SettingsDocument(
            ...
            new XElement(W + "printPostScriptOverText")));
        ...
        Read(saved).DoNotDisplayPageBoundaries.Should().BeTrue();
    }
