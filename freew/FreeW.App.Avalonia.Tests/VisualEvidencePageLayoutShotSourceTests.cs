...
public sealed class VisualEvidencePageLayoutShotSourceTests
{
    ...
    [Fact]
    public void PageLayoutShot_CropsTrackedChangesToTheWordComparablePageSurface()
    {
        var source = File.ReadAllText(RepositoryFile("freew", "tools", "FreeW.PageLayoutShot", "Program.cs"));

        source.Should().MatchRegex("ShouldCaptureWordComparablePageSurface\\([\\s\\S]*?string.Equals\\(scenarioId, \"f2-tracked-changes\", StringComparison.OrdinalIgnoreCase\\)");
        source.Should().Contain("CropToDocumentPageSurface(");
    }

    [Fact]
    public void PageLayoutShot_CropsCommentsToTheWordComparablePageSurface()
    {
        var source = File.ReadAllText(RepositoryFile("freew", "tools", "FreeW.PageLayoutShot", "Program.cs"));

        source.Should().MatchRegex("ShouldCaptureWordComparablePageSurface\\([\\s\\S]*?string.Equals\\(scenarioId, \"f2-comments\", StringComparison.OrdinalIgnoreCase\\)");
        source.Should().Contain("CropToDocumentPageSurface(");
    }

    [Fact]
    public void PageLayoutShot_CropsOutOfBodyCommentsToTheWordComparablePageSurface()
    {
        var source = File.ReadAllText(RepositoryFile("freew", "tools", "FreeW.PageLayoutShot", "Program.cs"));

        source.Should().MatchRegex("ShouldCaptureWordComparablePageSurface\\([\\s\\S]*?string.Equals\\(scenarioId, \"f2-outofbody-comments\", StringComparison.OrdinalIgnoreCase\\)");
        source.Should().Contain("VisualEvidenceOutputPath(outDir, \"f2-outofbody-comments\", 1)");
        source.Should().Contain("FreeWVisualEvidenceDocumentFactory.BuildOutOfBodyCommentsReviewDocument");
    }

    [Fact]
    public void PageLayoutShot_CropsBorderWatermarkToTheWordComparablePageSurface()
    {
        var source = File.ReadAllText(RepositoryFile("freew", "tools", "FreeW.PageLayoutShot", "Program.cs"));

        source.Should().MatchRegex("ShouldCaptureWordComparablePageSurface\\([\\s\\S]*?string.Equals\\(scenarioId, \"page-composition-border-watermark\", StringComparison.OrdinalIgnoreCase\\)");
        source.Should().Contain("CropToDocumentPageSurface(");
    }
    ...
    public void PageLayoutShot_CropsReviewCompareToTheWordComparablePageSurface()
    {
        ...
        source.Should().MatchRegex("ShouldCaptureWordComparablePageSurface\\([\\s\\S]*?string.Equals\\(scenarioId, \"review-compare-visual-proof\", StringComparison.OrdinalIgnoreCase\\)");
        source.Should().Contain("CropToDocumentPageSurface(");
    }
    ...
    public void PageLayoutShot_CropsReviewCombineToTheWordComparablePageSurface()
    {
        ...
        source.Should().MatchRegex("ShouldCaptureWordComparablePageSurface\\([\\s\\S]*?string.Equals\\(scenarioId, \"review-combine-visual-proof\", StringComparison.OrdinalIgnoreCase\\)");
        source.Should().Contain("CropToDocumentPageSurface(");
    }
