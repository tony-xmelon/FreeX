...
public static class DocumentMerge
{
    ...
    {
        // InlineImage is mutable (its size can change), so give the clone its own instance; the PNG
        // bytes are immutable content and may be shared.
        Image = source.Image is { } img ? new InlineImage(img.PngBytes, img.WidthPt, img.HeightPt) : null,
