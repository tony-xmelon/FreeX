...
public sealed class FidelityRenderCompositeTests
{
    ...
    private static RenderTargetBitmap RenderComposite(TextDocument doc)
    {
        ...
        // Layer 1b: Word's single fixed-size VML watermark shape.
    ...
    [StaFact]
    public void CompositeRender_TextWatermark_UsesOneCenteredShape()
    {
        var bitmap = RenderWatermarkPage(
        ...
        HasNonWhitePixelsInRegion(pixels, stride, new Rect(180, 300, 460, 420))
            .Should().BeTrue("Word centers one VML watermark shape on the page");
        HasNonWhitePixelsInRegion(pixels, stride, new Rect(0, 0, 120, 120))
            .Should().BeFalse("a text watermark must not repeat as a tiled corner pattern");
    }
