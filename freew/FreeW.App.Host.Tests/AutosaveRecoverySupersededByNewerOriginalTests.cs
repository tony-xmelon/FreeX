...
/// <summary>
/// r141: FreeW's recovery planners must apply the same "supersede by newer original" filter FreeX's
...
/// recovering it would silently overwrite newer on-disk work with stale autosaved content.
/// </summary>
public sealed class AutosaveRecoverySupersededByNewerOriginalTests : IDisposable
{
    private readonly TestTemporaryDirectory _temporaryDirectory = new("FreeWSupersededRecoveryTests-");
    ...
    /// <summary>
    /// Reproduces the finding's scenario: a crash leaves a snapshot at 10:00, the user later reopens
    /// the original and saves normally at 10:15 (the original file's mtime is now newer than the
    /// snapshot). Neither <see cref="AutosaveRecoveryPlanner.PlanAll"/> nor
    ...
    {
        var originalPath = Path.Combine(RecoveryDir, "document.docx");
        ...
        var stale = CreateCandidate("stale-snap", snapshotTakenAt, originalPath, "Stale Doc");
    ...
    /// <summary>
    /// Sibling of the superseded case: a genuine crash-recovery snapshot (no newer save happened --
    /// the original is older than the snapshot, exactly what a real crash produces) must still be
    /// offered normally. Proves the filter targets only the superseded case, not every candidate with
    /// an OriginalFilePath.
    /// </summary>
    [Fact]
    public void PlanAll_StillOffersASnapshotNewerThanItsOriginal()
    {
        ...
        var fresh = CreateCandidate("fresh-snap", snapshotTakenAt, originalPath, "Fresh Doc");
