...
/// <summary>
/// Uses the shared MCI path for narration and the Windows Runtime camera stack for local MP4 capture.
/// The synchronous recording contract is kept for the shared slideshow workflow; WinRT work is run on
/// a thread-pool thread so a WPF dispatcher is never used as the async completion context.
/// </summary>
internal sealed class WindowsHostRecordingCaptureEngine : IWindowsRecordingCaptureEngine
{
    ...
    public void BeginCapture(WindowsRecordingCaptureStartRequest request)
    {
        ...
        {
            // BeginCapture has no result channel. Keep the initialization error for CompleteCapture.
