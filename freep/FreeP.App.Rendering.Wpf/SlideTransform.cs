...
/// <summary>
/// Encapsulates the uniform-fit slide→screen transform used by <see cref="SlideCanvas"/>
/// and all interaction / adorner code that needs to map between slide DIP space and
/// WPF element (screen) space.
///
...
public sealed class SlideTransform
{
    ...
    public Point ScreenToSlide(double x, double y)
        => Scale == 0 ? new Point(0, 0) : new((x - OffsetX) / Scale, (y - OffsetY) / Scale);
    ...
    public static readonly SlideTransform Identity = new(1, 0, 0, 0, 0);
