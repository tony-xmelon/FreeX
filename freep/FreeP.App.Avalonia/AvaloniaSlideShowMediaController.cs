...
internal sealed class AvaloniaSlideShowMediaController
{
        ...
        bool showNarration = true)
    {
                     ...
                     && (_showNarration || shape.Media.IsVideo)))
        {
            ...
            try
            {
                ...
                {
                    StartPlayback(session, shape.Media);
                    if (view is not null)
                        view.IsVisible = true;
                }
        ...
        double canvasY)
    {
        ...
        {
            StartPlayback(slot.Session, slot.Media);
            if (slot.VideoView is not null)
    ...
    public bool TrySeek(uint shapeId, TimeSpan position)
    {
        ...
        return slot.Session.Seek(bounded);
    }
    ...
    private static void StartPlayback(IMediaPlaybackSession session, MediaInfo media)
    {
