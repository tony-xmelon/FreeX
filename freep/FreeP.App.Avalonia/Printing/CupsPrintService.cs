...
/// <summary>
/// Portable Avalonia printer adapter using the CUPS-compatible lpstat and lp commands.
/// Discovery and submission remain capability-backed; no queue is invented when CUPS is absent.
/// </summary>
public sealed class CupsPrintService : IPlatformPrintService
{
    ...
    public async Task<PrinterDiscoveryResult> DiscoverAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (!IsSupported)
            return new(PrinterDiscoveryStatus.Unavailable, [], null,
        ...
        try
        {
            ...
            var names = printersResult.StandardOutput
                ...
                .ToArray();
            if (names.Length == 0)
                return new(PrinterDiscoveryStatus.NoPrinters, [], null,
            ...
            var defaultName = defaultResult.Succeeded ? ParseDefaultPrinter(defaultResult.StandardOutput) : null;
            var printers = names
                .Select(name => new PrinterInfo(name,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        selection.Validate();
        if (cancellationToken.IsCancellationRequested)
            return new(PrintSubmissionStatus.Cancelled, selection.PrinterName,
                Message: "Print submission was cancelled.");
        if (!IsSupported)
            return new(PrintSubmissionStatus.Unavailable, selection.PrinterName,
                Message: "CUPS printing is available only on Linux and macOS hosts.");
        if (!File.Exists(pdfPath))
            return new(PrintSubmissionStatus.Failed, selection.PrinterName,
        ...
        try
        {
            ...
            return result.Succeeded
                ? new(PrintSubmissionStatus.Submitted, printer, result.StandardOutput.Trim())
                : new(PrintSubmissionStatus.Failed, printer,
                    Message: FormatProcessFailure("lp", result));
        }
        catch (OperationCanceledException)
        {
            return new(PrintSubmissionStatus.Cancelled, printer, Message: "Print submission was cancelled.");
        }
        catch (Exception ex) when (ex is not OutOfMemoryException)
        {
            return new(PrintSubmissionStatus.Failed, printer, Message: $"Print submission failed: {ex.Message}");
        }
    }
