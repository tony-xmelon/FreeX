...
public sealed class LocalizationKeyIntegrityTests
{
    ...
    public void AppSourceLocalizationKeys_AllExistInNeutralResources()
    {
        ...
        // Note: unlike FreeX.App.Avalonia, FreeP.App.Avalonia currently routes almost all UI text
        // through indirect resource-key fields (descriptor tables) rather than literal
        // UiText.Get("...") call sites, so `used` may legitimately be empty today. The sibling
        // RepresentativeCommonKeys_ResolveToRealNonSentinelText test below proves this sweep's
        // machinery (UiText.GetNeutralResourceKeys() + the [[key]] sentinel check) actually works.
