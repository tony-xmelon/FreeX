...
public sealed class SlideShowTestSupportOwnershipTests
{
    ...
    [
        ...
        "ActiveMediaCaptionForTest",
        "RefreshMediaCaptionsForTest",
        "get_LastMediaClickForTest",
    ...
    [Fact]
    public void TestAndValidationProjectsOwnConditionallyCompiledSlideshowAccess()
    {
        var root = TestWorkspaceFileLocator.FindDirectoryContainingFileFromBaseDirectory("FreeP.slnx");
        var hostDirectory = Path.Combine(root, "freep", "FreeP.App.Avalonia");
        var supportDirectory = Path.Combine(root, "freep", "TestSupport", "SlideShow.Avalonia");

        File.ReadAllText(Path.Combine(hostDirectory, "SlideShowWindow.cs"))
            .Should().NotContain("CreateValidationAccessAdapter");
        File.ReadAllText(Path.Combine(hostDirectory, "AvaloniaSlideShowMediaController.cs"))
        ...
        File.Exists(Path.Combine(supportDirectory, "AvaloniaSlideShowMediaController.TestAccess.cs")).Should().BeTrue();
