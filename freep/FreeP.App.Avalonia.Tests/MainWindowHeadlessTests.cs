...
public sealed class MainWindowHeadlessTests : IDisposable
{
    ...
    public async Task Backstage_print_actions_route_layout_selection_to_native_handoff()
    {
        ...
        PresentationPrintLayoutKind? expectedLayout = null;
        ...
        var ran = await OnUiThreadAsync(async () =>
        {
            ...
            try
            {
                ...
                expectedLayout = window.LastPrintBackstagePlan.LayoutChoices[0].Layout.Layout;
        ...
        printedRequest!.Layout.Should().Be(expectedLayout);
        printService.PdfBytes.Should().NotBeNullOrEmpty();
    }
    ...
    public void FreeP_hosts_link_the_same_canonical_owned_icon_family()
    {
        ...
        var brandAssets = File.ReadAllText(FindRepoFile(
            "shared", "Free.Shared.Shell", "BrandAssets.props"));

        brandAssets.Should().Contain("<BrandAssetBaseName>FreeP</BrandAssetBaseName>")
    ...
    public async Task SmartArt_layout_gallery_registers_extended_native_presets()
    {
        var ran = await OnUiThread(() =>
        {
            ...
            foreach (var commandId in new[]
            {
                ...
                SmartArtAuthoringPlanner.IncreasingCircleProcessLayoutCommandId,
    ...
    [Fact]
    public async Task ChartDisplayOptionsDialog_series_lines_uses_shared_planner()
    {
        ChartDisplayOptions? options = null;
        var ran = await OnUiThread(() =>
        {
            var window = new MainWindow(Array.Empty<string>());
            var chartShape = window.Editor.InsertChart(ChartType.ColumnStacked);
            chartShape.Chart!.SeriesLinesSpecified = true;
            window.Editor.Select(chartShape.Id);
        ...
        if (!ran) return;
        options.Should().NotBeNull();
        options!.ShowSeriesLines.Should().BeFalse();
    }
    ...
    [Fact]
    public async Task ChartTextOptionsDialog_can_target_chart_legend()
    {
        ChartTextOptions? options = null;
        var ran = await OnUiThread(() =>
        {
            var window = new MainWindow(Array.Empty<string>());
            var chartShape = window.Editor.InsertChart(ChartType.ColumnClustered);
            chartShape.Chart!.Legend = LegendPosition.Right;
            chartShape.Chart.LegendTextStyle = new ChartTextStyle { FontFamily = "Aptos", FontSizePt = 12 };
            window.Editor.Select(chartShape.Id);

            var dialog = new ChartTextOptionsDialog(window.Editor, ChartTextTarget.Legend);
            options = dialog.BuildCommitPlanForTests();
            dialog.Close();
        });
    ...
    [Fact]
    public async Task ChartSeriesOptionsDialog_can_start_at_requested_series()
    {
        ChartSeriesOptions? options = null;
        var ran = await OnUiThread(() =>
        {
            ...
            var dialog = new ChartSeriesOptionsDialog(window.Editor, initialSeriesIndex: 1);
            options = dialog.BuildCommitPlanForTests();
            dialog.Close();
        });
