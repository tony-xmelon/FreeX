...
public sealed class AppLanguageCatalogTests
{
    ...
    [Fact]
    public void GetAvailableLanguages_DiscoversFreePLocalizationSatelliteDirectories()
    {
        var baseDirectory = Path.Combine(Path.GetTempPath(), "FreePLanguageCatalogTests", Guid.NewGuid().ToString("N"));
        try
        {
            var satelliteDirectory = Path.Combine(baseDirectory, "fr-FR");
            Directory.CreateDirectory(satelliteDirectory);
            File.WriteAllText(Path.Combine(satelliteDirectory, "FreeP.App.Localization.resources.dll"), "");
