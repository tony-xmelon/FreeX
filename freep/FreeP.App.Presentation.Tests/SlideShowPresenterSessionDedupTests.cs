...
public sealed class SlideShowPresenterSessionDedupTests
{
    ...
    public void NativeHosts_KeepOnlyRendererProjectionInputAndWindowingResponsibilities()
    {
        ...
        foreach (var source in presenterFiles)
        {
            ...
            source.Should().Contain("SlideShowPresenterViewNativeBinding");
            source.Should().Contain("SlideShowPresenterViewHeaderComposition.Compose(");
            ...
            source.Should().Contain("control => control.IsKeyboardFocusWithin");
            source.Should().Contain("WindowStartupLocation = WindowStartupLocation.CenterOwner");
            source.Should().Contain("_coordinator.Surface");
            source.Should().Contain("_nativeBinding.Refresh()");
            source.Should().Contain("_nativeBinding.ExecuteAction(action)");
            source.Should().Contain("_nativeBinding.NotifyNotesTextChanged()");
            source.Should().Contain("_nativeBinding.CommitNotes()");
            source.Should().Contain("_nativeBinding.SelectPointerMode(mode.Value)");
            ...
            source.Should().NotContain("SlideShowPresenterViewRefreshPlan refresh");
            ...
            source.Should().NotContain("_pointerModeCombo.SelectedItem = plan.PointerMode;");
