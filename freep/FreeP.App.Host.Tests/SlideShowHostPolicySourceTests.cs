...
public sealed class SlideShowHostPolicySourceTests
{
    ...
    public void WpfSlideShowWindow_ExecutesAnimationStepsThroughSharedPlaybackPlans()
    {
        ...
        source.Should().Contain("_runtime.AnimationRendererSession.PlanFrame(");
        source.Should().Contain("PlayShapeAnimation(element, operation);");
        source.Should().Contain("if (operation.SuppressBaseBeforePlayback)");
        source.Should().Contain("AttachReveal(sb, operation);");
        ...
        source.Should().Contain("shapePlan.FontStyleShape is { } fontStyleShape");
        source.Should().Contain("SlideShowAnimationPlaybackTargetKind.FontStyle => _animFontStyleElements.GetValueOrDefault(operation.ShapeId)");
        ...
        source.Should().Contain("_animFontSizeElements");
        source.Should().Contain("shapePlan.FontSizeShape is { } fontSizeShape");
        source.Should().Contain("SlideShowAnimationPlaybackTargetKind.FontSize => _animFontSizeElements.GetValueOrDefault(operation.ShapeId)");
